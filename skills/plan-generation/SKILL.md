...
- The executor runs in a **network-disabled sandbox** with no access to GitHub or any remote. `acceptanceCriteria` must reference ONLY workspace-verifiable artifacts — code, tests, files, and build/test output present in the diff. NEVER make a GitHub/network side effect an acceptance criterion: no "update the issue Workpad comment", "post a PR comment", "close the issue", "push the branch", or any `gh`/`git push`/`curl`/API call. The pipeline performs all GitHub writes (issue comments, the Workpad, PRs) from the main process — they are not the executor's job and an executor can never satisfy them.
