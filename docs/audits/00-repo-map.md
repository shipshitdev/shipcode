...
Desktop process architecture: main entry `apps/desktop/src/main/index.ts` (48 flat files + `ipc/`), preload exposes a frozen `{invoke, on}` bridge (`contextIsolation: true`, `nodeIntegration: false`, `sandbox: true`), and `app.enableSandbox()` enforces Chromium sandboxing for every renderer. The main window also disables Node.js in workers/subframes, mixed content, and `<webview>`. Strict CSP `default-src 'none'` applies in production, and navigation is locked to the bundled origin (`apps/desktop/src/main/index-helpers.ts:146-240`). **202 `ipcMain.handle` registrations** across 15 domain handler files, all wrapped by a timing/Sentry decorator in `src/main/ipc.ts`. Largest: register-project-handlers.ts (61), register-pipeline-handlers.ts (37), register-github-handlers.ts (26).
...
10. ~~Docs/README staleness.~~ **Fixed 2026-07-02**: CLI README now documents all 14 commands; root README diagram and docs phase tables no longer reference "Opus 4.6" (the current pinned Claude phase model is Fable 5 per `PINNED_MODEL_DEFAULTS`).
...
2. **02 — Security audit.** Secrets-at-rest (move Telegram/Discord tokens to `safeStorage`), IPC input validation sampling across the 202 handlers, srt sandbox failure modes, worktree path-escape review (`assertWorkspaceSafe` coverage), gitleaks full-history scan. The BrowserWindow sandbox finding was resolved by enabling renderer sandboxing globally and pinning the full main-window posture in focused tests.
