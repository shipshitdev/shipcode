# Agent setup for `shipcode`

This is a deepsec scanning workspace. Project `shipcode` was just registered
(target: `..`). Setup is incomplete — `data/shipcode/INFO.md`
...
2. **Fill in `data/shipcode/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id shipcode
pnpm deepsec process --project-id shipcode
```
