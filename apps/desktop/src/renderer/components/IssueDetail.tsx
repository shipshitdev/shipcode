...
import {
	PlanViewer, ReviewViewer, Badge, Button, Textarea,
	X, ExternalLink, Select, SelectContent, SelectItem, SelectTrigger, SelectValue,
	getStatusBadgeVariant, ChevronLeft, Maximize2, Minimize2,
} from '@shipcode/ui'
...
export function IssueDetail({ expanded = false }: { expanded?: boolean }) {
	...
	const issueBadges = (
		<div className="flex flex-wrap gap-1.5">
			<Badge variant={getStatusBadgeVariant(activeIssue.pipelineStatus)} className="text-[11px] uppercase font-semibold">
		...
		<div className="mb-5">
			...
			<div className="grid grid-cols-2 gap-2">
				{[
					...
					<div key={role} className="flex flex-col gap-1 rounded-md border border-border bg-secondary p-2">
						<span className="text-[10px] font-medium uppercase tracking-wide text-muted">{role}</span>
						{editable ? (
							<Select value={activeIssue.executorModel} onValueChange={(v: string) => handleExecutorChange(v as 'claude' | 'codex' | 'openrouter')}>
							...
							<Badge variant="default" className="w-fit font-mono normal-case tracking-normal">{model}</Badge>
						)}
					</div>
				))}
			</div>
		...
		<div className="mb-5">
			...
			<div className="grid grid-cols-2 gap-2">
				...
				{thread.worktreeBranch && (
					<div className="flex flex-col gap-1 rounded-md border border-border bg-secondary p-2">
						<span className="text-[10px] font-medium uppercase tracking-wide text-muted">Branch</span>
						<span className="truncate font-mono text-[11px] text-primary">{thread.worktreeBranch}</span>
					</div>
				)}
				...
				<div className="flex flex-col gap-1 rounded-md border border-border bg-secondary p-2">
					...
					<span className="text-[11px] font-medium text-primary">
						{thread.totalCostUsd === 0 ? '$0.00' : thread.totalCostUsd < 0.005 ? '< $0.01' : `$${thread.totalCostUsd.toFixed(2)}`}
					</span>
					{thread.totalTokensPrompt > 0 && (
						<span className="text-[10px] text-muted">
							{((thread.totalTokensPrompt + thread.totalTokensCompletion) >= 1000
								...
								: String(thread.totalTokensPrompt + thread.totalTokensCompletion))} tokens
						</span>
					)}
				</div>
			</div>
	...
	const planHistorySection = planHistory.length > 0 ? (
		<div className="mb-5">
				...
				{planHistory.map(plan => {
					...
					return (
						<div key={plan.id} className={`rounded-md border ${isExpanded ? 'border-border' : 'border-transparent'}`}>
							...
							>
								<span className="font-mono text-xs font-semibold text-muted">v{plan.version}</span>
								...
								<div className="border-t border-border p-3">
									...
									{!plan.structured && (
										<div className="overflow-x-auto">
											<pre className="whitespace-pre-wrap text-xs text-secondary">{plan.rawOutput}</pre>
										</div>
									)}
								</div>
							)}
				...
				<div className="flex min-w-0 flex-1 flex-col overflow-hidden">
					...
					<div className="flex-1 overflow-y-auto p-6">
						...
						{activeThreadId && planHistory.length === 0 && (
							<div className="mb-5">
								<p className="py-4 text-center text-[13px] text-muted">Pipeline is running — waiting for plan generation...</p>
							</div>
						)}
					</div>
				</div>
	...
	return (
		<div className="h-full flex flex-col overflow-y-auto bg-primary">
			...
			<div className="flex-1 overflow-y-auto p-4">
				...
				{canRerun && (
					<div className="mb-5 py-6 text-center">
						<p className="mb-3 text-muted">Pipeline failed. You can re-run it from the beginning.</p>
						<Button
							size="lg"
							variant="outline"
							onClick={handleRerun}
