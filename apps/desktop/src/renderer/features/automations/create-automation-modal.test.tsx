...
describe('CreateAutomationModal', () => {
  ...
  it('loads an existing automation and maps unknown cron schedules to custom', async () => {
    act(() => {
    ...
    expect(await screen.findByDisplayValue('Existing automation')).toBeInTheDocument();
  ...
  it('submits edits for an existing automation', async () => {
    ...
    const nameInput = await screen.findByDisplayValue('Existing automation');
    fireEvent.change(nameInput, { target: { value: 'Edited automation' } });
  ...
  it('shows submit failures and keeps the modal open', async () => {
    invokeMock.mockImplementation(async (channel: string) => {
      if (channel === 'project:list-visible') return [makeProject()];
      if (channel === 'automations:create') throw new Error('create failed\nfull trace');
      return null;
    });
  ...
  it('shows format failures without clearing the typed prompt', async () => {
    invokeMock.mockImplementation(async (channel: string) => {
      if (channel === 'project:list-visible') return [makeProject()];
      if (channel === 'ai:format-automation') throw new Error('format failed\nfull trace');
      return null;
    });
    ...
    expect(await screen.findByText('format failed')).toBeInTheDocument();
    expect(promptInput).toHaveValue('do thing');
  });

  it('supports Escape close and Cmd+Enter submit shortcuts', async () => {
    const { rerender } = renderWithProviders();

    await screen.findByText('My Repo');
    fireEvent.keyDown(screen.getByText('New automation'), { key: 'Escape' });
    expect(useAppStore.getState().createAutomationModalOpen).toBe(false);
    ...
    await screen.findByText('My Repo');
    fireEvent.change(screen.getByPlaceholderText(/Daily smoke test/), {
      target: { value: 'Shortcut automation' },
    });
    fireEvent.change(screen.getByPlaceholderText(/What should this run do\?/), {
      target: { value: 'do thing' },
    });
    fireEvent.keyDown(screen.getByText('New automation'), { key: 'Enter', metaKey: true });

    await waitFor(() => {
      expect(invokeMock).toHaveBeenCalledWith(
        'automations:create',
        expect.objectContaining({ name: 'Shortcut automation' }),
      );
    });
  });
