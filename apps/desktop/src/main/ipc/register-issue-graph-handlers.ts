...
export function notifyIssueGraphPipelinePhaseChange(input: {
  ...
  for (const [runId, run] of activeGroupedRuns) {
    if (run.projectId !== completedIssue.projectId || !run.selectedIssueIds.includes(completedIssue.id)) {
      continue;
    }
    ...
    for (const issueId of newlyReadyIssueIds) {
      ...
      const issue = deps.queries.githubIssues.list(run.projectId).find((entry) => entry.id === issueId);
      if (!issue) continue;
