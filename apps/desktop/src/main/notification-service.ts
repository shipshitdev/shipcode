...
function kindToEventFlag(kind: NotificationKind): keyof import('@shipcode/shared').NotificationEventToggles {
  switch (kind) {
    case 'awaiting_approval': return 'awaitingApproval'
    case 'failed': return 'failed'
    case 'completed': return 'completed'
    case 'verification_exhausted': return 'verificationExhausted'
  }
}
...
function buildCopy(kind: NotificationKind, thread: Thread): { title: string; body: string } {
  ...
  switch (kind) {
    case 'awaiting_approval':
      return { title: 'Plan ready for review', body: `${label} — tap to approve or request changes` }
    case 'failed':
    ...
    case 'verification_exhausted':
      return { title: 'Verification failed', body: `${label} hit the retry limit and needs a human` }
  }
}
