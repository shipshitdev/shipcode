...
```json
{
  ...
  "verifyCommands": ["bun run typecheck", "bun run test"],
  "runtimeQa": {
    "server": {
    ...
    "testCommands": ["bun run test:e2e"],
    "discoverAgentTests": true
  },
  "envFiles": [{ "source": ".env.local", "required": true }],
  "setupBeforeVerify": false,
  "testingContext": "Tests use Vitest and Playwright."
}
```

Project detection can prefill setup and verify commands from common repo profiles. Runtime QA starts the configured dev server inside the feature worktree, runs browser/E2E commands, and also supports generated visual QA when a PRD includes a `## QA State` contract.
