...
describe('planFileChangeSchema', () => {
  ...
  it('rejects empty description', () => {
    expect(() => planFileChangeSchema.parse({ ...validFileChange, description: '' })).toThrow();
  });

  it('rejects invalid action', () => {
    expect(() => planFileChangeSchema.parse({ ...validFileChange, action: 'move' })).toThrow();
  });
...
describe('shipCodePlanSchema', () => {
  ...
  it('accepts all valid complexity values', () => {
    for (const c of ['low', 'medium', 'high']) {
      expect(shipCodePlanSchema.parse({ ...validPlan, estimatedComplexity: c })).toBeTruthy();
    }
  });
...
describe('reviewFindingSchema', () => {
  ...
  it('rejects invalid severity', () => {
    expect(() => reviewFindingSchema.parse({ ...validFinding, severity: 'blocker' })).toThrow();
  });

  it('accepts all valid severities', () => {
    for (const s of ['critical', 'major', 'minor', 'nit']) {
      expect(reviewFindingSchema.parse({ ...validFinding, severity: s })).toBeTruthy();
    }
  });

  it('rejects invalid category', () => {
    expect(() => reviewFindingSchema.parse({ ...validFinding, category: 'style' })).toThrow();
  });

  it('accepts all valid categories', () => {
    for (const c of ['correctness', 'security', 'performance', 'design', 'missing']) {
      expect(reviewFindingSchema.parse({ ...validFinding, category: c })).toBeTruthy();
    }
  });

  it('rejects empty description', () => {
    expect(() => reviewFindingSchema.parse({ ...validFinding, description: '' })).toThrow();
  });
});
...
describe('planReviewSchema', () => {
  ...
  it('rejects invalid decision', () => {
    expect(() => planReviewSchema.parse({ ...validReview, decision: 'defer' })).toThrow();
  });

  it('accepts all valid decisions', () => {
    for (const d of ['approve', 'request_changes', 'reject']) {
      expect(planReviewSchema.parse({ ...validReview, decision: d })).toBeTruthy();
    }
  });

  it('rejects invalid confidence', () => {
    expect(() => planReviewSchema.parse({ ...validReview, confidence: 'very_high' })).toThrow();
  });

  it('accepts all valid confidence values', () => {
    for (const c of ['high', 'medium', 'low']) {
      expect(planReviewSchema.parse({ ...validReview, confidence: c })).toBeTruthy();
    }
  });
...
describe('criteriaCheckSchema', () => {
  ...
  it('rejects empty criterion', () => {
    expect(() => criteriaCheckSchema.parse({ ...validCriteriaCheck, criterion: '' })).toThrow();
  });

  it('rejects empty evidence', () => {
    expect(() => criteriaCheckSchema.parse({ ...validCriteriaCheck, evidence: '' })).toThrow();
  });

  it('rejects non-boolean passed', () => {
    expect(() => criteriaCheckSchema.parse({ ...validCriteriaCheck, passed: 'yes' })).toThrow();
  });
...
describe('verificationIssueSchema', () => {
  ...
  it('rejects invalid severity', () => {
    expect(() => verificationIssueSchema.parse({ ...validIssue, severity: 'critical' })).toThrow();
  });

  it('accepts all valid severities', () => {
    for (const s of ['blocker', 'warning']) {
      expect(verificationIssueSchema.parse({ ...validIssue, severity: s })).toBeTruthy();
    }
  });

  it('rejects empty description', () => {
    expect(() => verificationIssueSchema.parse({ ...validIssue, description: '' })).toThrow();
  });
});
...
describe('verificationResultSchema', () => {
  ...
  it('rejects empty threadId', () => {
    expect(() => verificationResultSchema.parse({ ...validVerification, threadId: '' })).toThrow();
  });

  it('rejects empty planId', () => {
    expect(() => verificationResultSchema.parse({ ...validVerification, planId: '' })).toThrow();
  });
  ...
  it('accepts all valid result values', () => {
    for (const r of ['passed', 'failed']) {
      expect(verificationResultSchema.parse({ ...validVerification, result: r })).toBeTruthy();
    }
  });

  it('rejects empty summary', () => {
    expect(() => verificationResultSchema.parse({ ...validVerification, summary: '' })).toThrow();
  });
