...
export function projectSlug(projectPath: string): string {
  const base = path.basename(projectPath).replace(/[^a-zA-Z0-9._-]/g, '-').slice(0, 48)
 ...
 */
export function resolveWorktreeParent(projectPath: string, worktreeRoot: string | null | undefined): string {
