...
describe('editTool', () => {
  it('replaces a unique substring', async () => {
    ...
    const res = await editTool.execute({ path: 'f.txt', oldString: 'world', newString: 'there' }, ctx)
  ...
  it('returns error if oldString is not found', async () => {
    ...
    const res = await editTool.execute({ path: 'f.txt', oldString: 'missing', newString: 'x' }, ctx)
    expect(res.ok).toBe(false)
