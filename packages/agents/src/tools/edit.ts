...
export const editTool: Tool<EditInput> = {
  ...
  parameters: {
    ...
    properties: {
      ...
      oldString: { type: 'string', description: 'Text to find. Must match exactly once unless replaceAll is set.' },
  ...
  async execute(input: EditInput, ctx: ToolContext): Promise<ToolResult> {
    ...
    if (input.replaceAll) {
      ...
      updated = content.slice(0, first) + input.newString + content.slice(first + input.oldString.length)
    }
