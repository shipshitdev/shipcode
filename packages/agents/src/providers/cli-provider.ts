  ...
  return {
    ...
    async generate(req: ProviderRequest): Promise<ProviderResponse> {
      ...
      return {
        ...
        ...(result.exitCode === 127
          ? { providerError: { kind: 'binary_missing' as const, message: 'claude CLI not found on PATH', retryable: false } }
          : result.exitCode === 130
  ...
  return {
    ...
    async generate(req: ProviderRequest): Promise<ProviderResponse> {
      ...
      return {
        ...
        resolvedModel: 'codex',
        ...(result.exitCode === 127
          ? { providerError: { kind: 'binary_missing' as const, message: 'codex CLI not found on PATH', retryable: false } }
          : result.exitCode === 130
            ? { providerError: { kind: 'network' as const, message: 'aborted', retryable: false } }
            : {}),
      }
