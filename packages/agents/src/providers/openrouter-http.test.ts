...
import { OpenRouterClient, OpenRouterError, _internals } from './openrouter-http'
...
describe('OpenRouterClient', () => {
  ...
  describe('error handling', () => {
    it('401 becomes non-retryable auth error', async () => {
      ...
      const client = buildClient()
      await expect(
        client.chat({ model: 'm', messages: [{ role: 'user', content: 'x' }] }, new AbortController().signal),
  ...
  describe('abort handling', () => {
    ...
    it('mid-stream abort cancels the reader', async () => {
      ...
      const body = new ReadableStream<Uint8Array>({
        start(controller) {
          controller.enqueue(encoder.encode('data: {"choices":[{"index":0,"finish_reason":null,"delta":{"content":"part"}}]}\n\n'))
