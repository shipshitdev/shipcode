...
/**
 * Literal first line of every plan comment body. Used by `upsertIssueCommentByMarker`
 * to find and edit the existing plan comment in place instead of posting a new one
 * on every revision.
 */
export const PLAN_COMMENT_MARKER = '## ShipCode Implementation Plan';

export function formatPlanComment(plan: ShipCodePlan): string {
  const sections: string[] = [];

  sections.push(PLAN_COMMENT_MARKER);
  sections.push('');
  sections.push(`> [!NOTE]`);
