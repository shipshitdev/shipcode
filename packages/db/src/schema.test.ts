...
describe('migrateV2', () => {
	...
	it('creates v2 tables and adds thread columns', () => {
		...
		const row = db.prepare('SELECT github_issue_number, autonomous FROM threads WHERE id = ?').get('t1') as any
...
describe('migrateV3', () => {
	...
	it('adds last_status_label column and reclassifies queued to todo', () => {
		...
		const row = db.prepare('SELECT pipeline_status FROM github_issue_cache WHERE id = ?').get('i1') as any
