...
- [ ] **Pre-flight:** Vincent adds `HOMEBREW_TAP_TOKEN` PAT to `shipshitdev/shipcode` repo secrets (scoped: `contents:write` on `homebrew-shipcode`) — needed for first release to auto-update cask SHA256
...
- [ ] **Still pending from session 22:** HOMEBREW_TAP_TOKEN PAT, first-release artifact filename verification, official cask submission
