...
## Summary
Session 38: Open PR button in IssueDetail (shell:open-external), archived issues in Settings > Archived tab (Projects | Issues tabs), `listArchived` DB method, 2 new IPC channels. 3 commits on develop.
Session 37: Repo Context Phase 2 — `context-generator.ts`, 3 IPC channels (`context:list/generate/read`), Context Files section in ProjectSettingsModal. `ContextFileInfo` in shared. 14/14 typecheck clean.
Session 36: Per-phase model + reasoning effort settings — all 4 pipeline phases (planner/reviewer/executor/verifier) now have independent provider, OpenRouter model ID, and reasoning effort controls in Settings > Pipeline. Claude thinking tokens and OpenRouter reasoning effort are no longer hardcoded.
Session 35: Unified Terminal Event System -- canonical TerminalEvent stream across all 3 providers (Claude/Codex/OpenRouter), reasoning/thinking output, animated spinner, blockquote text styling, clickable links. PRs #36 and #37 merged. Worktree/branch cleanup. CI green.

Session 34: TerminalDrawer infinite loop fix, PipelineStatus stepper overflow fix (labels hidden on inactive steps), Kanban progress bar shimmer animation wired up.
Session 32: Fix Switch/toggle invisible thumb — `bg-text-primary` was a no-op (not a background utility); replaced with state-specific `bg-accent-foreground` / `bg-[var(--text-primary)]` for proper contrast in ON and OFF states.
...
## Session 32: Fix Toggle/Switch Component Rendering
...
| Layer | Components |
|-------|------------|
| Frontend | `packages/ui/src/primitives/switch.tsx` |

### What was done

- [x] Diagnosed invisible Switch thumb — root cause was `bg-text-primary` resolving to nothing (only `text-primary` is defined as a `@utility` for `color:`, not `background-color:`)
- [x] Fixed thumb to use state-conditional colors: `data-[state=checked]:bg-accent-foreground` (dark `#050607` thumb on white track) and `data-[state=unchecked]:bg-[var(--text-primary)]` (light `#f4f4f5` thumb on dark track)

### Files changed

- `packages/ui/src/primitives/switch.tsx` — removed `bg-text-primary` from Thumb, replaced with `data-[state=checked]:bg-accent-foreground data-[state=unchecked]:bg-[var(--text-primary)]`

### Key decisions

- **Decision:** Use state-specific thumb colors rather than adding `@utility bg-text-primary` to app.scss
  - **Context:** In checked state, both track (`#fafafa` accent) and thumb (`#f4f4f5` text-primary) were near-identical white — no contrast
  - **Rationale:** State-specific classes solve both the "no background" bug and the contrast problem in one change; adding a generic bg-text-primary utility would only fix the transparent thumb but still leave white-on-white in checked state

### Mistakes and fixes

- **Mistake:** `bg-text-primary` used on thumb — only `text-primary` is defined as `@utility text-primary { color: ... }`, not as a background utility, so no background was applied → **Fix:** replaced with explicit `data-[state=checked]:bg-accent-foreground data-[state=unchecked]:bg-[var(--text-primary)]`

### Next steps

- [ ] Visual QA in running app — confirm ON toggle shows dark thumb on white track, OFF shows light thumb on dark track

## Session 34: TerminalDrawer infinite loop, stepper overflow, progress bar shimmer
...
| Layer | Components |
|-------|------------|
| Frontend | `apps/desktop/src/renderer/components/TerminalDrawer.tsx`, `packages/ui/src/PipelineStatus.tsx`, `packages/ui/src/KanbanBoard.tsx`, `apps/desktop/src/renderer/styles/app.scss` |

### What was done

- [x] Fixed TerminalDrawer infinite re-render loop (black screen crash on pipeline start)
- [x] Fixed PipelineStatus stepper overflowing / cutting off steps 6-8 in narrow IssueDetail panel
- [x] Wired up shimmer animation on active Kanban progress bars

### Files changed

- `apps/desktop/src/renderer/components/TerminalDrawer.tsx` — hoisted `EMPTY_STREAM` constant to module level; `?? []` inline fallback was creating a new array reference on every Zustand `getSnapshot` call, triggering infinite re-renders via `useSyncExternalStore`
- `packages/ui/src/PipelineStatus.tsx` — hide label for future (inactive) steps; only completed + active steps show text label; tightened connector from `w-6` to `w-4`; reduced button x-padding to `px-1.5`. Reduces total stepper width from ~750px to ~350px.
- `packages/ui/src/KanbanBoard.tsx` — added shimmer overlay `<span>` inside the progress fill div for active pipeline states (planning/reviewing/revising/executing/verifying/shipping); uses `animate-slide-progress` class; shimmer is absent for completed/failed/awaiting_approval
- `apps/desktop/src/renderer/styles/app.scss` — registered `--animate-slide-progress: slide-progress 2s linear infinite` in `@theme` block so Tailwind generates the `animate-slide-progress` utility; updated comment on the keyframe

### Key decisions

- **Decision:** Hide labels on future/inactive stepper steps rather than adding `overflow-x-auto`
  - **Context:** 8 steps × ~70px + 7 connectors ≈ 750px; IssueDetail panel is narrower, causing clip
  - **Rationale:** Labels on future steps add noise; showing only circle numbers is common UX pattern and keeps the stepper scannable without scroll

- **Decision:** Shimmer only on active agent states, not completed/awaiting_approval/failed
  - **Context:** Progress bar uses different colors per state — green=done, amber=needs human, red=failed
  - **Rationale:** Static states are final or awaiting human input; animation would be misleading noise

- **Decision:** Module-level `EMPTY_STREAM` constant for Zustand selector fallback
  - **Context:** Zustand uses `useSyncExternalStore`; `getSnapshot` must return same reference if value is unchanged
  - **Rationale:** Inline `?? []` always produces a new array reference → Zustand sees a change → re-render → repeat → crash

### Mistakes and fixes

- **Mistake:** Tried to rename the `slide-progress` CSS comment to clarify intent before wiring — not needed, just needed `@theme` registration
  - **Fix:** Registered `--animate-slide-progress` in the existing `@theme inline` block

### Next steps

- [ ] QA: verify shimmer is visible on Kanban cards during active pipeline run
- [ ] QA: verify stepper shows all 8 steps without overflow in IssueDetail
- [ ] QA: confirm TerminalDrawer no longer crashes on pipeline start

---

## Session 35: Unified Terminal Event System
...
flowchart TD
    ...
    subgraph Main["Main Process"]
        CN["ClaudeNormalizer"]
        CXN["CodexNormalizer"]
        CB["onTerminalEvent callback"]
    end
    subgraph Renderer
        Store["canonicalTerminalStream"]
        TD["TerminalDrawer"]
    end
    Claude -->|PTY| CN -->|IPC| Store
    Codex -->|PTY| CXN -->|IPC| Store
    OR -->|emit| CB -->|IPC| Store
    Store --> TD
```
...
| Layer | Components |
|-------|------------|
| Types | `terminal-events.ts` (NEW), `pipeline/types.ts`, `providers/types.ts` |
| Normalizers | `claude-normalizer.ts` (NEW), `codex-normalizer.ts` (NEW) |
| Providers | `cli-provider.ts`, `openrouter-http.ts`, `openrouter-provider.ts`, `openrouter-execute.ts` |
...
- [x] Canonical TerminalEvent type (10 kinds: text, thinking, tool_start, tool_end, turn_start, turn_end, lifecycle, raw, error, done)
- [x] Claude/Codex normalizers parse NDJSON in main process, emit canonical events
- [x] OpenRouter SSE consumer emits thinking + text deltas; execute harness emits turn/tool events
- [x] Claude CLI: --max-thinking-tokens 32000 for plan/revision/verify
- [x] Codex CLI: default model_reasoning_effort=high so reasoning always appears
- [x] TerminalDrawer rewritten to single event-driven renderer (removed old NDJSON parsing)
- [x] Animated braille spinner with phase-aware labels (Thinking/Working/Reviewing)
- [x] Blockquote styling: cyan left border for agent text, magenta for thinking
- [x] Suppress [exit 0] noise from Codex tool calls
- [x] Clickable terminal links via @xterm/addon-web-links
- [x] Merged feat/terminal-scope, cleaned worktrees + stale remote branches
- [x] PRs #36 and #37 merged, CI green
- [x] Saved feedback: always PR to develop, never direct merge

### Files changed

- `packages/agents/src/terminal-events.ts` - NEW: TerminalEvent type
- `packages/agents/src/providers/normalizers/claude-normalizer.ts` - NEW
- `packages/agents/src/providers/normalizers/codex-normalizer.ts` - NEW
- `packages/agents/src/index.ts` - re-exports
- `packages/agents/src/providers/types.ts` - onTerminalEvent callback
- `packages/agents/src/providers/cli-provider.ts` - thinking flags + default reasoning
- `packages/agents/src/providers/cli-provider.test.ts` - updated snapshots
- `packages/agents/src/providers/openrouter-http.ts` - reasoning SSE + onDelta
- `packages/agents/src/providers/openrouter-provider.ts` - wire reasoning
- `packages/agents/src/providers/openrouter-provider.test.ts` - fix assertions
- `packages/agents/src/providers/openrouter-execute.ts` - tool loop events
- `packages/pipeline/src/types.ts` - terminal:event variant
- `packages/pipeline/src/pipeline.ts` - onTerminalEvent + test phase raw
- `apps/desktop/src/main/ipc.ts` - normalizer registry
- `apps/desktop/src/renderer/stores/app-store.ts` - canonicalTerminalStream
- `apps/desktop/src/renderer/hooks/useIpc.ts` - terminal:event + lifecycle
- `apps/desktop/src/renderer/components/TerminalDrawer.tsx` - full rewrite
- `apps/desktop/package.json` - @xterm/addon-web-links

### Key decisions

- **Decision:** Normalize in main process, not renderer
  - **Context:** OpenRouter has no PTY; events must originate from main process
  - **Rationale:** Keeps renderer dumb; all providers feed one canonical stream

- **Decision:** PRs to develop, not direct merges
  - **Context:** Vincent corrected direct merge of feat/terminal-scope
  - **Rationale:** PR workflow ensures CodeRabbit review + audit trail

- **Decision:** Suppress exit 0, only show failures
  - **Context:** Every Codex command printed [exit 0], flooding terminal
  - **Rationale:** Success is the default; only failures are actionable

### Mistakes and fixes

- **Mistake:** Direct-merged feat/terminal-scope instead of PR -> **Fix:** saved feedback memory, used PRs for all subsequent work
- **Mistake:** expect.anything() doesn't match undefined in vitest -> **Fix:** used chatSpy.mock.calls[0][0] pattern
- **Mistake:** Zustand naming collision with existing terminalEvents -> **Fix:** named new slice canonicalTerminalStream (caught by adversarial review)

### Next steps

- [ ] QA: run Claude pipeline -- verify thinking blocks as magenta blockquote
- [ ] QA: run Codex review -- verify no [exit 0], reasoning as dim italic
- [ ] QA: run OpenRouter pipeline -- verify terminal not blank
- [ ] QA: verify clickable URLs, animated spinner, cyan blockquote text
- [ ] Consider removing legacy terminalEventsByThread from Zustand

---

## Session 36: Per-phase model + reasoning effort settings
...
```mermaid
flowchart TD
    Settings["Settings > Pipeline"] --> PhaseRow["PhaseModelRow (x4)"]
    PhaseRow --> ProviderSelect["Provider Select\n(claude/codex/openrouter)"]
    PhaseRow --> ORModelInput["OpenRouter model ID input\n(conditional)"]
    PhaseRow --> ReasoningSelect["Reasoning effort Select\n(low/medium/high)"]
    ReasoningSelect --> DB["SQLite settings\n(4 effort fields)"]
    DB --> Pipeline["pipeline.ts\nphaseHints.reasoningEffort"]
    Pipeline --> Claude["cli-provider.ts\n--max-thinking-tokens\n(high=32000, medium=8000, low=omit)"]
    Pipeline --> Codex["cli-provider.ts\n-c model_reasoning_effort=<effort>"]
    Pipeline --> OR["openrouter-provider.ts\nreasoning: { effort }"]
```
...
| Layer | Components |
|-------|------------|
| Frontend | `SettingsPanel.tsx`, `SettingsSidebar.tsx` |
| Backend | `pipeline.ts`, `cli-provider.ts`, `openrouter-provider.ts` |
| Data | `types.ts` (AppSettings), `constants.ts`, `settings.ts` (DB queries) |

### What was done

- [x] Back button in SettingsSidebar made full-width (matches nav item style)
- [x] Added per-phase provider selectors for all 4 phases (planner/reviewer/executor/verifier) — only executor was shown before
- [x] Provider options constrained per phase: codex not offered for plan/verify; openrouter not offered for execute
- [x] OpenRouter model ID input shown conditionally per phase when provider = openrouter
- [x] Added `plannerReasoningEffort`, `executorReasoningEffort`, `verifierReasoningEffort` to `AppSettings` (joined existing `reviewerReasoningEffort`)
- [x] DB layer parses and validates all 4 effort fields (consolidated into loop in `set()`)
- [x] `pipeline.ts` passes correct per-phase reasoning effort to all 4 phase calls (plan/revision/execute/verify); review already had it
- [x] Claude CLI: `claudeThinkingTokens()` helper maps effort to `--max-thinking-tokens` (high=32000, medium=8000, low=omit flag entirely); wired into plan/revision/verify/execute
- [x] OpenRouter: reads `req.phaseHints?.reasoningEffort` instead of hardcoded `'high'`; omits `reasoning` and `include_reasoning` when effort is `'low'`
- [x] Settings UI: `PhaseModelRow` component now includes reasoning effort Select for every phase; old codex-only conditional row removed

### Files changed

- `apps/desktop/src/renderer/components/SettingsSidebar.tsx` — back button full-width
- `apps/desktop/src/renderer/components/SettingsPanel.tsx` — `PhaseModelRow` gains `reasoningEffortValue`/`onReasoningEffortChange`; all 4 phase call sites updated; old reviewer-only reasoning row removed
- `packages/shared/src/types.ts` — 3 new reasoning effort fields on `AppSettings`
- `packages/shared/src/constants.ts` — defaults `'high'` for the 3 new fields
- `packages/db/src/queries/settings.ts` — parse all 4 effort fields; validate via loop
- `packages/pipeline/src/pipeline.ts` — plan/revision pass `plannerReasoningEffort`, execute passes `executorReasoningEffort`, verify passes `verifierReasoningEffort`
- `packages/agents/src/providers/cli-provider.ts` — `claudeThinkingTokens()` helper; Claude execute phase also gets thinking tokens
- `packages/agents/src/providers/openrouter-provider.ts` — reasoning effort from phaseHints, not hardcoded

### Key decisions

- **Decision:** Add reasoning effort to all 4 phases, not just reviewer
  - **Context:** User pointed out Claude and OpenRouter also have reasoning, not just Codex
  - **Rationale:** Each phase is independent; planner/verifier may need different depth than reviewer/executor

- **Decision:** Claude execute phase now also supports `--max-thinking-tokens`
  - **Context:** Previously thinking was only on plan/revision/verify; execute was tool-only
  - **Rationale:** User wants full control; thinking on execute can improve code quality; harmless if set to low

- **Decision:** OpenRouter `low` effort omits `reasoning` and `include_reasoning` entirely
  - **Context:** OpenRouter's API skips reasoning when the field is absent vs. `effort: 'low'`
  - **Rationale:** Cleaner than sending `effort: 'low'` which may still charge reasoning tokens on some models

### Mistakes and fixes

- **Mistake:** `PhaseModelRow` call sites not updated after adding new required props → TS errors
  - **Fix:** Updated all 4 call sites with `reasoningEffortValue` and `onReasoningEffortChange`

- **Mistake:** Stale TS LSP diagnostics fired during edits (e.g., "Cannot find plannerReasoningEffort") because `@shipcode/shared` build hadn't refreshed
  - **Fix:** Ran `bun run typecheck` at the end — full 14-task pass confirmed all real errors resolved

### Next steps

- [ ] QA: open Settings > Pipeline, verify all 4 phase rows show provider + reasoning effort
- [ ] QA: change planner reasoning to `low`, run a pipeline, confirm no `--max-thinking-tokens` flag in Claude CLI invocation (check logs)
- [ ] QA: change a phase to OpenRouter with `low` effort, confirm `reasoning` field absent from HTTP request
- [ ] Consider adding a visual label/hint explaining what each reasoning level means (e.g., "high = extended thinking, slower + more costly")
- [ ] Commit and PR to develop

## Session 37: Repo Context Phase 2 — Generate & Manage Context Files
...
```mermaid
flowchart TD
    User -->|opens Project Settings| ProjectSettingsModal
    ProjectSettingsModal -->|context:list| IPC
    IPC -->|listContextFiles| ContextGenerator
    ContextGenerator -->|statSync each file| ContextDir[".agents/context/*.md"]
    ContextDir --> IPC
    IPC --> ProjectSettingsModal
    ProjectSettingsModal -->|renders file list| UI

    User -->|clicks Generate Context| handleGenerateContext
    handleGenerateContext -->|context:generate| IPC2[IPC]
    IPC2 -->|generateContextFiles| ContextGenerator2[ContextGenerator]
    ContextGenerator2 -->|reads| SourceFiles["README.md, package.json, CLAUDE.md"]
    ContextGenerator2 -->|stdin prompt| ClaudeCLI["claude -p (via runClaudeWithStdin)"]
    ClaudeCLI -->|shipcode-context JSON| ContextGenerator2
    ContextGenerator2 -->|writes 4 files| ContextDir2[".agents/context/"]
    ContextGenerator2 --> IPC2
    IPC2 --> handleGenerateContext
    handleGenerateContext -->|refetchContext| ProjectSettingsModal
```
...
| Layer | Components |
|-------|------------|
| Frontend | `ProjectSettingsModal.tsx` — new Context Files section with file list + Generate button |
| Backend | `ipc.ts` — 3 new handlers: `context:list`, `context:generate`, `context:read` |
| Agents | `context-generator.ts` (new) — `listContextFiles`, `readContextFile`, `generateContextFiles` |
| Shared | `types.ts` — `ContextFileInfo` interface; `ipc-channels.ts` — 3 new channel types |

### What was done

- [x] Adversarial review of plan via Codex before implementation — found 2 blockers (ContextFileInfo in wrong package, missing explicit type param on useQuery)
- [x] Updated plan with all fixes before implementing
- [x] Added `ContextFileInfo` interface to `packages/shared/src/types.ts` (not agents) so renderer can import it
- [x] Added `ContextFileInfo` to import in `packages/shared/src/ipc-channels.ts`
- [x] Added 3 channel type entries to `IpcInvokeChannels`: `context:list`, `context:generate`, `context:read`
- [x] Created `packages/agents/src/context-generator.ts` with `listContextFiles`, `readContextFile`, `generateContextFiles`
- [x] `runClaudeWithStdin` copied verbatim from `prd-generator.ts` (no modification to existing file)
- [x] Full defensive JSON envelope parsing (try/catch fallback to raw stdout) matching `prd-generator.ts`
- [x] Line-by-line `shipcode-context` fence extraction matching `extractPrd` pattern
- [x] Path traversal guard in `readContextFile` (validates name against known 4 filenames)
- [x] Exported new functions from `packages/agents/src/index.ts`
- [x] Added `generateContextFiles`, `listContextFiles`, `readContextFile` to value import block in `ipc.ts`
- [x] Added 3 IPC handlers near `ai:enhance-prd` in `ipc.ts`
- [x] Added `ContextFileInfo` import, 2 new state vars, reset in useEffect, `useQuery<ContextFileInfo[]>` with explicit type param, `handleGenerateContext`, and JSX section to `ProjectSettingsModal.tsx`
- [x] 14/14 typecheck clean; pre-existing `cli-provider.test.ts` failure confirmed pre-existing (not introduced by this session)

### Files changed

- `packages/shared/src/types.ts` — added `ContextFileInfo` interface
- `packages/shared/src/ipc-channels.ts` — added `ContextFileInfo` to imports; added 3 channel types
- `packages/agents/src/context-generator.ts` — **NEW**: full context generator module
- `packages/agents/src/index.ts` — exported `generateContextFiles`, `listContextFiles`, `readContextFile`, `ContextGenerateResult`
- `apps/desktop/src/main/ipc.ts` — added 3 function imports + 3 IPC handlers
- `apps/desktop/src/renderer/components/ProjectSettingsModal.tsx` — added Context Files section

### Key decisions

- **Decision:** `ContextFileInfo` lives in `packages/shared`, not `packages/agents`
  - **Context:** Renderer bundle cannot import from `packages/agents`; adversarial review caught this as a blocker
  - **Rationale:** `shared` is the correct boundary for types that cross the IPC boundary to the renderer

- **Decision:** Copy `runClaudeWithStdin` verbatim rather than extracting to shared util
  - **Context:** `prd-generator.ts` is already shipped; extracting would require modifying it
  - **Rationale:** Avoids touching stable code; duplication is ~50 lines and both sites are stable

- **Decision:** `context:generate` handler returns `{ success, error }` rather than throwing
  - **Context:** Generation takes ~30s; structured failure is more useful to the renderer than an IPC error
  - **Rationale:** Matches AI operation convention (same pattern as the generate handler design intent)

- **Decision:** Use `projectId` (not `projectPath`) as the IPC arg
  - **Context:** All other handlers in the codebase use `projectId` and resolve path server-side
  - **Rationale:** Consistency; path is an implementation detail of the main process

### Mistakes and fixes

- **Mistake:** Initial plan exported `ContextFileInfo` from `packages/agents` — renderer can't import that package
  - **Fix:** Adversarial review caught this; moved `ContextFileInfo` to `packages/shared/src/types.ts` before implementation

- **Mistake:** `git stash pop` failed after pre-existing test check because `ProjectSidebar.tsx` had unrelated local changes
  - **Fix:** `git checkout` the conflicting file first, then `git stash pop`

### Next steps

- [ ] Manual QA: open Project Settings → Context Files section renders with 4 rows (all `—`)
- [ ] Manual QA: click "Generate Context" → button shows "Generating…" → after ~30s green dots + file sizes appear
- [ ] Manual QA: run a pipeline on a project with context files — confirm context injected into plan phase output
- [ ] Fix pre-existing `cli-provider.test.ts` failure: `buildClaudeArgs` execute phase test expects `--max-thinking-tokens 32000` not present but it is (separate issue, pre-dates this session)
- [ ] Commit and PR to develop
- [ ] Phase 3 (future): `context:read` IPC is wired but has no UI consumer — add inline viewer/editor to the modal

## Session 38: Open PR Button + Archived Issues Tabs
...
```mermaid
flowchart TD
    Thread -->|githubPrNumber + githubRepo| IssueDetail
    IssueDetail -->|click "Open PR"| shell:open-external
    shell:open-external -->|hardened handler| Browser[System browser]

    User -->|Settings > Archived| SettingsPanel
    SettingsPanel -->|Projects tab| ArchivedProjects[Existing archived projects list]
    SettingsPanel -->|Issues tab| ArchivedIssues[github:list-archived]
    ArchivedIssues -->|Restore| github:unarchive-issue
    github:unarchive-issue -->|clearArchivedAt| DB
    DB -->|archived_at = NULL| KanbanBoard
```
...
| Layer | Components |
|-------|------------|
| Frontend | `IssueDetail.tsx` (Open PR button), `SettingsPanel.tsx` (tabs + issues section) |
| Backend | `ipc.ts` (2 new handlers: `github:list-archived`, `github:unarchive-issue`) |
| Data | `github-issues.ts` (`listArchived()` method) |
| Shared | `ipc-channels.ts` (2 new channel types) |

### What was done

- [x] Replaced static `#<number>` PR chip in IssueDetail with a clickable "Open PR" Button
- [x] Uses existing `shell:open-external` IPC (hardened: https-only, github.com allowlist)
- [x] Guard: button only renders when both `githubPrNumber` AND `githubRepo` are set
- [x] Added `GitHubIssueQueries.listArchived()` — global list ordered by `archived_at DESC`
- [x] Added `github:list-archived` and `github:unarchive-issue` IPC channels + handlers
- [x] Restore = `clearArchivedAt` locally only; does NOT reopen on GitHub
- [x] Refactored SettingsPanel Archived section from two stacked sections into Tabs (Projects | Issues)
- [x] Committed as 3 logical commits on develop: per-phase reasoning effort backlog + session 38 features + switch fix
- [x] Typecheck clean (6/6), tests passing (29/29)

### Files changed

- `apps/desktop/src/renderer/components/IssueDetail.tsx` — Open PR button replacing static PR chip
- `apps/desktop/src/renderer/components/SettingsPanel.tsx` — Tabs import, archived issues query/mutation, tabbed JSX
- `apps/desktop/src/main/ipc.ts` — `github:list-archived` and `github:unarchive-issue` handlers
- `packages/shared/src/ipc-channels.ts` — 2 new channel type entries
- `packages/db/src/queries/github-issues.ts` — `listArchived()` method

### Key decisions

- **Decision:** Use `shell:open-external` + URL construction instead of `gh pr view --web`
  - **Context:** Vincent asked why not use gh CLI
  - **Rationale:** repo + PR number already on the thread object; string interpolation is instant vs. spawning a subprocess. The hardened IPC handler already exists.

- **Decision:** No confirm modal before opening PR URL
  - **Context:** Vincent asked if we should match the "archive all" confirm pattern
  - **Rationale:** Opening a URL is harmless and reversible (just a browser tab). Confirm modals are for destructive actions only.

- **Decision:** No AI code reviewer phase added
  - **Context:** Vincent asked about adding a reviewer step after PR creation
  - **Rationale:** Verify phase already catches implementation issues before PR opens; human reviewers on GitHub handle code quality. An AI re-review would be redundant noise.

- **Decision:** Restore = local `clearArchivedAt` only, no GitHub `gh issue reopen`
  - **Context:** Archive closes the issue on GitHub; restore could be confusing if it also reopened
  - **Rationale:** User can reopen on GitHub manually. The refresh-issues handler already handles the inverse (GitHub reopen → local unarchive).

- **Decision:** Tabs (Projects | Issues) instead of separate sidebar sections or two stacked sections
  - **Context:** Vincent asked about merging both archive lists into one surface
  - **Rationale:** Tabs are the simplest consolidation — no new SettingsSection values, no sidebar changes, reuses existing Radix Tabs from `@shipcode/ui`. Single `archived` section stays clean.

### Mistakes and fixes

- **Mistake:** Used `window.electron.invoke` for shell:open-external call
  - **Fix:** All IPC in renderer uses `window.shipcode.invoke`; corrected before typecheck

### Next steps

- [ ] Manual QA: complete a pipeline run → verify "Open PR" button appears in IssueDetail and opens correct GitHub URL
- [ ] Manual QA: archive an issue → Settings > Archived > Issues tab → Restore → verify board shows it again
- [ ] PR develop → master when ready to release (develop is ahead of master)
