...
Overview dashboard pagination and row consistency polish, docs dev-port/header-logo fixes, PR `#49` review plus merge via `gh` fallback, provider warning popovers, forensic log analysis, CPU performance overhaul fixing 99% CPU saturation during pipeline EXECUTE phase, worktree reuse and notification cleanup, and stale worktree branch cleanup with 4 features re-implemented fresh on develop plus Vite node:path externalization fix.
...
## Session 11: Stale Worktree Branch Cleanup And Feature Re-implementation
...
```mermaid
flowchart TD
  A[5 stale ship/* worktree branches] --> B{Merge into develop?}
  B -- Conflicts: 20+ files --> C[Abandon merge approach]
  C --> D[Re-implement 4 features fresh on develop]
  D --> D1[Agent 1: Pipeline scheduler]
  D --> D2[Agent 2: PRD attachments + image drop]
  D --> D3[Agent 3: Approval badges + dashboard]
  D --> D4[Agent 4: Pipeline smoke test + formatTokenCount]
  D1 & D2 & D3 & D4 --> E[Biome lint fixes]
  E --> F[Commit 1ec921a on develop]
  F --> G[Push worktree branches, open PRs #54-#58]
  G --> H[PRs useless - massive divergence diffs]
  H --> I[Close all 5 PRs]
  I --> J[Remove all 5 worktrees]
  J --> K[Delete local + remote branches]
  K --> L[Close GitHub issues #23, #42, #51, #52]
  L --> M[Fix Vite node:path externalization warning]
```
...
| Layer | Components |
|-------|------------|
| Desktop main | `pipeline-scheduler.ts`, `pipeline-bridge.ts`, `ipc/prd-attachments.ts`, `ipc/register-support-handlers.ts` |
| Desktop renderer | `CreateIssueModal.tsx`, `ProjectSidebar.tsx`, `CostsView.tsx` |
| Shared package | `ipc-channels.ts`, `types.ts`, `format-token-count.ts`, `index.ts`, `package.json` |
| DB package | `queries/dashboard.ts`, `queries/settings.ts` |
| Pipeline package | `pipeline.github-issue.smoke.test.ts` |
| Git package | `worktree.ts` |
| UI package | `index.ts` (ImageIcon export) |
| Git branches | 5 `ship/*` branches (local + remote) |
| GitHub | PRs #54-#58, Issues #23, #38, #42, #51, #52 |

### What was done

**Branch cleanup:**
- [x] Attempted merge/cherry-pick of 5 stale worktree branches — all failed with 20+ conflicts
- [x] Re-implemented 4 unique features fresh on develop using parallel agents reading old diffs
- [x] Fixed all Biome lint issues (noNonNullAssertion, noExplicitAny, useSemanticElements, etc.)
- [x] Committed as `1ec921a` on develop
- [x] Pushed worktree branches to remote, opened PRs #54-#58 for comparison
- [x] Confirmed PRs were useless (massive structural divergence), closed all 5
- [x] Removed all 5 worktrees from `~/.shipcode/worktrees/shipcode-5097cb/`
- [x] Deleted all 5 local and remote `ship/*` branch refs
- [x] Closed GitHub issues #23, #42, #51, #52 with implementation notes (#38 already closed)

**Vite node:path fix:**
- [x] Diagnosed `Module "path" has been externalized` warning — `worktree-path.ts` (uses `node:path`) was barrel-exported from `@shipcode/shared`, pulled into renderer by Vite
- [x] Removed `worktree-path` from main `@shipcode/shared` barrel export
- [x] Added dedicated `@shipcode/shared/worktree-path` subpath export in `package.json`
- [x] Updated 2 Node-only consumers (`@shipcode/git`, `@shipcode/db`) to use subpath import
- [x] Verified shared, db, and git package tests all pass

### Features re-implemented

1. **Pipeline Scheduler** (#23) — `PipelineScheduler` class with `startOrQueue()` slot management, `onSlotFreed()` queue drain, integrated with `pipeline-bridge.ts` terminal phases
2. **PRD Image Attachments** (#52) — Session lifecycle (create/stage/remove/clear), magic byte validation (PNG/JPEG/GIF/WebP), `CreateIssueModal` drag-and-drop zone using `<Button>` from `@shipcode/ui`, IPC handlers in `register-support-handlers.ts`
3. **Approval Badges** (#51) — Yellow `<Badge variant="warning">` in `ProjectSidebar` for pending approvals, `pendingApprovalsByProject` dashboard query
4. **Pipeline Smoke Test + Token Format** — End-to-end GitHub issue pipeline test (plan → review → awaiting_approval), `formatTokenCount` refactored with proper formatting (em dash for 0, M/k suffixes)

### Key decisions

- **Decision:** Re-implement features fresh instead of merging stale branches
  - **Context:** All 5 branches shared base `dae81ad` which diverged 20+ files from current develop
  - **Rationale:** Cherry-pick/merge produced unresolvable conflicts; re-implementing against current code patterns was faster and produced cleaner results

- **Decision:** Use parallel agents (4 concurrent) for independent feature domains
  - **Context:** Each feature touched non-overlapping files
  - **Rationale:** 4 features implemented in the time of 1; no merge conflicts between agents

- **Decision:** Split `worktree-path` out of `@shipcode/shared` barrel via subpath export
  - **Context:** `worktree-path.ts` imports `node:path` which Vite externalizes for browser, causing console warning
  - **Rationale:** Only 2 Node-only packages use it; subpath export is clean and doesn't break any consumer
...
- `apps/desktop/src/main/ipc/prd-attachments.ts`
...
**Modified files:**
- `apps/desktop/src/main/pipeline-bridge.ts` — `awaiting_approval` triggers slot-freed
- `apps/desktop/src/main/ipc/register-support-handlers.ts` — 4 prd-attachments IPC handlers
- `apps/desktop/src/renderer/components/CreateIssueModal.tsx` — attachment UI, drop zone, session lifecycle
- `apps/desktop/src/renderer/components/ProjectSidebar.tsx` — approval badge
- `apps/desktop/src/renderer/components/CostsView.tsx` — use shared `formatTokenCount`
- `packages/shared/src/index.ts` — removed `worktree-path` barrel export
- `packages/shared/package.json` — added `./worktree-path` subpath export
- `packages/shared/src/types.ts` — `pendingApprovalsByProject` in DashboardStats
- `packages/shared/src/ipc-channels.ts` — `StagedPrdAttachment`, 4 prd-attachment channels
- `packages/shared/src/format-token-count.ts` — refactored formatting
- `packages/db/src/queries/dashboard.ts` — per-project approval count query
- `packages/db/src/queries/settings.ts` — import from `@shipcode/shared/worktree-path`
- `packages/git/src/worktree.ts` — import from `@shipcode/shared/worktree-path`
- `packages/ui/src/index.ts` — `ImageIcon` re-export

### Mistakes and fixes

- **Mistake:** Tried merge/cherry-pick first, wasted time on 20+ conflict resolutions
  - **Fix:** Abandoned merge, re-implemented fresh — much faster and cleaner

- **Mistake:** Drop zone used raw `<div role="button">` instead of `<Button>` from `@shipcode/ui`
  - **Fix:** Changed to `<Button variant="ghost">` with `cn()` for className merging

- **Mistake:** `buf.slice` deprecation in prd-attachments
  - **Fix:** Changed to `buf.subarray()`

- **Mistake:** `worktree-path` in shared barrel caused Vite externalization warning in renderer
  - **Fix:** Moved to dedicated subpath export `@shipcode/shared/worktree-path`

### Verification

- All package tests pass (shared 18, db 21, git 8)
- Biome lint clean
- Shared package builds successfully with new subpath export
- CSP warning confirmed as standard Electron dev-mode noise

### Next steps

- [ ] QA: verify `Module "path" externalized` warning no longer appears in dev console
- [ ] QA: test image drop in CreateIssueModal — drop PNG, verify staging, remove, cancel cleanup
- [ ] QA: verify yellow approval badges appear in ProjectSidebar when issues are awaiting_approval
- [ ] QA: start 4+ pipelines simultaneously to verify scheduler queues beyond slot limit
