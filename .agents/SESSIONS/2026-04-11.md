...
- Session 34: Mega session — docs alignment (Track A/B/C), CLI rename to @shipshitdev/shipcode, release workflow (manual-dispatch + OIDC Trusted Publisher), apps/web OpenRouter value prop, KanbanBoard 'use client' fix, Projects v2 backfill sync (idempotent addIssueToProject, sync-to-project-board IPC + ProjectSettingsModal button, auto-attach on start-issue, gh `project` scope check in onboarding + HealthBanner), GH issue #26 created, worktree cleanup, plan file corrections. 30+ files touched, 14/14 typecheck, 267 agents tests green.
...
- Session 28: Stale inbox notification fix + inline plan formatting — auto-dismiss thread notifications on pipeline restart; inline expanded plan cards now use `resolveClientSidePlan` fallback same as full-screen dialog
- Session 27: Per-project `githubProjectUrl` override + Project Settings modal — Kanban `board` quick-link can now be pointed at a real GitHub Projects v2 board via a new modal from the `⋯` menu; new `migrateV10`, `validateGithubProjectUrl`, shared `clampError`, 24 new tests, 14/14 typecheck clean
- Session 26: List view phase breakdown — list view now mirrors Kanban's column/section structure (Planning/Reviewing/Executing/Verifying sub-sections with model labels, purple agent tone on active rows)
- Session 25: Terminal thread switcher dropdown — replaced #N pill tabs with a dropdown on the issue title when 2+ pipelines are running
...
## Session 25: Terminal thread switcher dropdown
...
- [x] Replaced the `#7 #8 #11` pill buttons in the terminal header right-side with a dropdown on the issue title in the left header
- [x] When 1 running issue: issue title remains a plain button (opens issue detail), no change
- [x] When 2+ running issues: issue title becomes a `DropdownMenu` trigger showing `#N Title ˅`; dropdown lists all running issues with the active one highlighted; selecting an item calls `setTerminalThread` + `selectIssue`
- [x] Imported `ChevronDown`, `DropdownMenu`, `DropdownMenuContent`, `DropdownMenuItem`, `DropdownMenuTrigger` from `@shipcode/ui`
- [x] Dropdown opens `side="top"` (above the terminal bar), `align="start"` (left-aligned with title)

### Files changed

- `apps/desktop/src/renderer/components/TerminalDrawer.tsx` — replaced right-side pill tabs with left-side dropdown trigger on issue title; added dropdown imports

### Key decisions

- **Decision:** Dropdown opens upward (`side="top"`) over the terminal
  - **Rationale:** Terminal bar is at the bottom of the window — downward would go off-screen
- **Decision:** Keep single-issue path as a plain button (no dropdown chrome)
  - **Rationale:** Dropdown affordance is only useful when there are multiple choices; avoids visual noise in the common case
...
- [ ] Manual QA: run 2+ concurrent pipelines, verify dropdown appears and switches terminal output correctly
- [ ] Address pre-existing TypeScript errors in `useIpc.ts`, `IssueDetail.tsx`, `threads.ts`, `run.ts`, `pipeline.ts` (carried from prior sessions)
- [ ] Manual QA #1 — nested Edit PRD modal stacking
- [ ] Manual QA #5 — Generate with AI → Create & Plan end-to-end

---

## Session 26: List view phase breakdown — mirror Kanban sections in IssueListView
...
| Layer | Components |
|-------|------------|
| UI package | `packages/ui/src/KanbanBoard.tsx` (list view section only) |

### Context

Vincent screenshot-compared Kanban vs list view: the Kanban `Agent Loop` column stacks 4 phase sub-sections (Planning · `sonnet 4.6` / Reviewing · `gpt 5.4` / Executing · `<executor>` / Verifying · `sonnet 4.6`) with purple `agent` tone on active cards, while the list view collapsed everything into a single flat `In Progress` bucket with a generic accent dot. Asked: "why don't we have the same view of the agent status (purple) in the issues list view?"

### What was done

- [x] Replaced `LIST_GROUPS` constant with iteration over `COLUMNS` directly — single source of truth with Kanban; `LIST_COLUMN_LABEL` maps `agent → 'In Progress'` and `human → 'Blocked'` for display
- [x] Added `LIST_COLUMN_DROP_ID` mapping only `todo → 'todo'` and `agent → 'agent:planning'`; `human`/`done` are non-droppable
- [x] Added `rowToneFor(status)` helper that returns `'agent' | 'danger' | 'warning' | 'default'` driven by `ACTIVE_STATUSES` / `failed` / `awaiting_approval`
- [x] Rewrote `DraggableListRow` — drops `dotClass` prop; derives tone from `pipelineStatus`; renders `border-l-2` with `border-agent/60`, `border-danger/60`, or `border-warning/60` + matching low-opacity bg; selected state bumps border/bg opacity instead of switching color
- [x] Active rows now render a pulsing dot (`animate-ping rounded-full bg-agent opacity-60` over solid `bg-agent`) matching the Kanban card's top-right pulse
- [x] New `ListSectionBlock` component — mirrors Kanban's `SectionBlock` header shape: phase label + `MODEL_DISPLAY[agentLabel]` · separator on `columnKey === 'agent'`, count pill with danger/warning tone for `failed`/`awaiting` sections
- [x] Executor model resolved per-issue via `issues[0]?.executorModel`, falling back to any currently-executing thread's executor, then `'claude'` — matches Kanban's behavior so the Executing row header shows the real model even when that sub-section is momentarily empty
- [x] `DroppableListGroup` now takes an optional `dropId` (use `'__noop__'` sentinel with `disabled: true` when missing) so `human`/`done` columns render without a dnd-kit droppable at all
- [x] `IssueListView` iterates `COLUMNS`, uses `col.sections` branch to render phase sub-sections when present, otherwise flat rows for `todo`/`done`
- [x] Preserved group-level drop targets: dragging onto `In Progress` still hits `agent:planning` (start/rerun pipeline), dragging onto `Todo` still hits `todo` (failed→queued retry) — no changes to `handleDragEnd`
- [x] `bun run typecheck` — 14/14 packages green

### Files changed

- `packages/ui/src/KanbanBoard.tsx` — deleted `LIST_GROUPS` / `LIST_GROUP_DROP_ID`; added `LIST_COLUMN_LABEL` / `LIST_COLUMN_DROP_ID` / `RowTone` / `rowToneFor`; rewrote `DraggableListRow` with tone-driven border/bg + pulsing active dot; added `ListSectionBlock`; updated `DroppableListGroup` to accept optional `dropId`; rewrote `IssueListView` to iterate `COLUMNS` with `col.sections` branch

### Key decisions

- **Decision:** Use `COLUMNS` as the single source of truth for both views rather than maintaining a parallel `LIST_GROUPS` constant
  - **Context:** The Kanban already defines the phase→section→agent mapping on `COLUMNS`; list view was duplicating the shape with different labels
  - **Rationale:** New phases or agents only need to be added in one place; keeps list and Kanban mechanically in sync

- **Decision:** `border-l-2` stripe instead of full-card border for row tone
  - **Context:** Kanban cards use a full border (`border-agent/40`) because they're boxed; list rows are flat horizontal bars where a full border would feel heavy
  - **Rationale:** Left stripe communicates the same tone without competing with the row's horizontal alignment; the 2px width stays readable at the row's short vertical height

- **Decision:** Pulsing `bg-agent` dot for active rows (matching Kanban's top-right pulse) instead of keeping the static accent dot
  - **Context:** The original flat-list dot was `bg-accent` for all active statuses — Vincent explicitly called out the missing purple agent signal
  - **Rationale:** Matches the Kanban exactly, reuses the same classes, and the animate-ping gives the same "thread is alive right now" cue

- **Decision:** Preserve group-level drop targets on `Todo` and `Agent Loop`, not section-level
  - **Context:** Kanban makes the Planning sub-section droppable with `id='agent:planning'`; in the list view that would hide the drop zone behind a narrow sub-header
  - **Rationale:** List view keeps the simpler UX — drop anywhere on the In Progress group to start/rerun — by routing the group-level `DroppableListGroup` to `agent:planning`. Same id, same `handleDragEnd` branch, no changes to drop semantics

- **Decision:** Fallback chain for executor model resolution in `ListSectionBlock` (`issues[0]?.executorModel` → `allIssues.find(executing)?.executorModel` → `'claude'`)
  - **Context:** Kanban's `SectionBlock` resolves per-section only (`issues[0]?.executorModel ?? 'claude'`), which shows "claude" when the Executing sub-section is momentarily empty between phases
  - **Rationale:** Adding a second fallback to any currently-executing thread in `allIssues` keeps the header accurate across phase transitions without requiring a state machine

### Mistakes and fixes

- **Mistake:** A stale mid-edit diagnostics snapshot reported `LIST_GROUPS` / `LIST_GROUP_DROP_ID` as still referenced after my `IssueListView` rewrite, plus unused-var warnings on `LIST_COLUMN_LABEL` / `LIST_COLUMN_DROP_ID` / `ListSectionBlock`
  - **Fix:** Ran `bun run typecheck` as ground truth — all 14 packages clean. Same pattern Session 15/21 documented: `tsc --noEmit` wins over LSP cache

### Next steps

- [ ] Manual QA: start a pipeline → verify list view shows the active thread under `In Progress → Planning · sonnet 4.6` with purple left bar + pulsing dot
- [ ] Manual QA: phase transitions → row moves between Planning/Reviewing/Executing/Verifying sub-sections
- [ ] Manual QA: drag a Todo onto the Agent Loop group → still starts pipeline
- [ ] Manual QA: drag a Failed row onto Todo → still resets to queued
- [ ] Manual QA: verify `human`/`done` groups render without dnd drop visuals (no accidental hover ring)
- [ ] Follow-up: consider applying the same border-l stripe treatment to `InboxView` rows for consistency with list view tone signal

---

## Session 27: Per-project `githubProjectUrl` override + Project Settings modal

**Status:** Complete (code + tests green; manual QA pending)

### Problem

The Kanban header `board` quick-link always linked to `https://github.com/<owner>/<repo>/projects` — the repo's *Projects tab* that lists linked Projects v2 boards, not a board itself. GitHub Projects v2 live under a user/org (`/users/<name>/projects/<n>` or `/orgs/<name>/projects/<n>`) and can span multiple repos, so we can't derive the real URL from `gitRemote` alone. Vincent surfaced this while clicking the button and landing on the wrong page.

### System Flow

```mermaid
flowchart LR
  Sidebar[ProjectSidebar ⋯ menu] -->|Settings| Modal[ProjectSettingsModal]
  Modal -->|validateGithubProjectUrl| Val[shared/github-url.ts]
  Modal -->|project:set-github-project-url| IPC[main/ipc.ts]
  IPC -->|updateGithubProjectUrl| DB[(projects.github_project_url)]
  ThreadPanel -->|githubProjectsUrl remote, override| Board[Kanban 'board' button]
  DB --> Project[Project row via project:get] --> ThreadPanel
```
...
| Layer | Components |
|-------|------------|
| Frontend | `ProjectSettingsModal.tsx` (new), `ProjectSidebar.tsx`, `ThreadPanel.tsx`, `App.tsx`, `app-store.ts` |
| Backend | `apps/desktop/src/main/ipc.ts` (new `project:set-github-project-url` handler) |
| Shared | `packages/shared/src/types.ts`, `github-url.ts`, `errors.ts` (new), `ipc-channels.ts`, `index.ts` |
| Data | `packages/db/src/schema.ts` (migrateV10), `queries/projects.ts`, `index.ts`, `test-helpers.ts` |

### What was done

- [x] Adversarial plan review via `codex:rescue` — first run wedged after 5min+, killed and re-dispatched with tighter 7-question focus, completed in 1m05s with 6 concrete defects
- [x] Patched the plan file with codex findings (migration V9→V10, destructured IPC payloads, sole `githubProjectsUrl` caller in `ThreadPanel`, no existing Settings menu item, modal mount in `App.tsx` not `ProjectSidebar`, no shared `clampError` helper)
- [x] Added `githubProjectUrl: string | null` to `Project` interface
- [x] New `migrateV10()` in `schema.ts` — nullable `github_project_url` column on `projects`, inline try/catch pattern matching V8
- [x] Wired `migrateV10` into `packages/db/src/index.ts` bootstrap chain and `test-helpers.ts` in-memory test DB
- [x] Added `ProjectQueries.updateGithubProjectUrl(id, url)` + updated `mapProject` to read `github_project_url`
- [x] Extended `githubProjectsUrl(remote, override?)` — override wins, empty/whitespace falls through, backward-compat preserved (no callers broke)
- [x] New `validateGithubProjectUrl` in `github-url.ts` — accepts `orgs/<org>/projects/<n>`, `users/<user>/projects/<n>`, `<owner>/<repo>/projects/<n>`; rejects http, non-github, issue/pull URLs, non-numeric project numbers; null/empty is a valid clear
- [x] New shared `clampError(raw, max = 280)` in `packages/shared/src/errors.ts` — first-line + truncate with ellipsis
- [x] New IPC handler `project:set-github-project-url` with destructured `{ projectId, url }` payload, validates via `validateGithubProjectUrl`, routes rejection reason through `clampError`, logs via `electron-log/main`
- [x] Added `project:set-github-project-url` to `IpcInvokeChannels` in `ipc-channels.ts`
- [x] App-store: `projectSettingsModalOpen`, `projectSettingsModalProjectId`, `openProjectSettingsModal(id)`, `closeProjectSettingsModal()`
- [x] New `ProjectSettingsModal.tsx` — `@shipcode/ui` primitives only (`Dialog`, `Input`, `Label`, `Button`), plain `useState`, read-only name/gitRemote/defaultBranch rows, editable `githubProjectUrl` input with `onBlur` inline validation, red border + inline error on invalid, clamped submit error toast, ⌘↩ shortcut, mutation invalidates all 4 project query keys
- [x] Mounted `<ProjectSettingsModal />` in `App.tsx` next to `<CreateIssueModal />`
- [x] Added `Settings` lucide icon + `DropdownMenuItem` in `ProjectSidebar.tsx` placed above Pin
- [x] `ThreadPanel.tsx:53` — pass `project?.githubProjectUrl` as second arg to `githubProjectsUrl()`
- [x] 20 new tests in `packages/shared/src/github-url.test.ts`
- [x] 4 new tests in `packages/db/src/queries/projects.test.ts`
- [x] Full repo typecheck green (14/14 packages)
- [x] Full test suites green: shared 114 ✓, db 136 ✓, desktop 22 ✓

### Files changed

**Shared:**
- `packages/shared/src/types.ts` — added `githubProjectUrl: string | null` to `Project`
- `packages/shared/src/github-url.ts` — extended `githubProjectsUrl(remote, override?)`; added `validateGithubProjectUrl` + `GithubProjectUrlValidation` type
- `packages/shared/src/errors.ts` (new) — `clampError` helper
- `packages/shared/src/index.ts` — barrel-exported `./errors`
- `packages/shared/src/ipc-channels.ts` — added `project:set-github-project-url` channel type
- `packages/shared/src/github-url.test.ts` (new) — 20 tests

**DB:**
- `packages/db/src/schema.ts` — added `migrateV10()`
- `packages/db/src/index.ts` — imported + called `migrateV10`
- `packages/db/src/test-helpers.ts` — imported + called `migrateV10`
- `packages/db/src/queries/projects.ts` — added `updateGithubProjectUrl`; updated `mapProject`
- `packages/db/src/queries/projects.test.ts` — 4 new tests

**Desktop main:**
- `apps/desktop/src/main/ipc.ts` — imported `validateGithubProjectUrl` + `clampError`; new `project:set-github-project-url` handler

**Desktop renderer:**
- `apps/desktop/src/renderer/stores/app-store.ts` — new modal state + open/close actions
- `apps/desktop/src/renderer/components/ProjectSettingsModal.tsx` (new)
- `apps/desktop/src/renderer/App.tsx` — mounted modal alongside `CreateIssueModal`
- `apps/desktop/src/renderer/components/ProjectSidebar.tsx` — `Settings` icon import + new dropdown item
- `apps/desktop/src/renderer/components/ThreadPanel.tsx:53` — threaded `project?.githubProjectUrl` into `githubProjectsUrl`

### Key decisions

- **Decision:** Store a single `githubProjectUrl` string override, not structured `{owner, ownerType, number}` fields
  - **Context:** Plan called out both options. We only need to *open* the board in a browser right now — no Projects v2 API calls
  - **Rationale:** Simplest possible schema. Can split into structured fields later when/if we actually query the GraphQL API
- **Decision:** Edit via a modal triggered from the existing `⋯` dropdown, not a `/project/:id/settings` route
  - **Rationale:** A route is premature for one field. Modal matches `CreateIssueModal` convention. Promote to a route if the surface grows past ~5 fields
- **Decision:** Added `clampError` to `packages/shared/src/errors.ts` rather than inlining in `main/ipc.ts`
  - **Context:** Codex review confirmed there's no existing shared helper; `CreateIssueModal.tsx:43` had a local copy. Repo memory calls out "clamp IPC errors" as a hard rule
  - **Rationale:** One helper reusable from both main and renderer beats two per-file copies. Existing handlers NOT retrofitted — out of scope
- **Decision:** Validator accepts all three Project URL shapes (orgs/users/repo-scoped classic) and trims whitespace
  - **Rationale:** Real users paste from browser URL bar, which includes all three variants
- **Decision:** Null/empty/whitespace is a *valid* clear, returns `{ok: true, value: null}`
  - **Rationale:** Users need a way to reset the override back to the repo Projects tab default

### Mistakes and fixes

- **Mistake:** First adversarial review dispatch via `codex:codex-rescue` — launched twice (background + `--wait`), both processes wedged after 2–5 minutes scanning broadly → **Fix:** Cancelled both, re-dispatched with a tightly-scoped 7-question prompt and a 400-word cap. Finished in 1m05s with 6 concrete defects. Lesson: bound the scope *and* the output length
- **Mistake:** Lost track of working directory after `cd packages/shared && bun run build` — next commands ran from `packages/shared/` instead of repo root → **Fix:** Switched to absolute paths for every subsequent `bun run`. Should have used absolute `cwd`-preserving commands from the start
- **Mistake:** LSP diagnostics flagged spurious "no exported member" errors for `@shipcode/ui` icons and claimed `ProjectSettingsModal` was unused after mounting it → **Fix:** Recognized this as the same stale-IDE-cache pattern from Sessions 15/21; trusted forced `bun run typecheck` (14/14 green) over inline diagnostics

### Root causes worth remembering

- **Plan claims are only trustworthy for last-verified context.** The first plan misstated migration number (V10 vs V9), IPC signature style (assumed positional, actual is destructured object), caller inventory (claimed `KanbanBoard` imports `githubProjectsUrl`, it receives the URL as a prop), modal mount location (`ProjectSidebar` vs `App.tsx`), and helper availability (`addColumnIfMissing` is local to V7, not shared). Always run adversarial review on non-trivial plans before implementing
- **GitHub Projects v2 are NOT nested under a repo.** They live under a user/org. `/<owner>/<repo>/projects` is the repo's "linked Projects" tab — a list, not a board. The fallback is still useful, but not a substitute for the real board URL
- **Codex rescue focus matters.** Broad "verify everything" prompts stall at 5+ minutes of grepping. Numbered-question prompts with a word cap completed in ~1 minute

### Next steps

- [ ] Manual QA: open `⋯` menu on any project → Settings → modal shows read-only name/remote/branch + editable URL field
- [ ] Manual QA: paste `https://github.com/orgs/shipshitdev/projects/1` → Save → Kanban `board` button links to that URL
- [ ] Manual QA: clear the field → Save → `board` button reverts to `${repoBase}/projects`
- [ ] Manual QA: paste malformed URL → inline red error on blur, Save stays disabled
- [ ] Manual QA: restart app → override persists (migrateV10 applied, mapProject reads column)

## Session 31: Docs alignment with production-ready desktop app

**Status:** Complete (Tracks A/B/C landed; docs build + full monorepo typecheck green; not yet committed)

### Problem

Vincent's prompt: "the desktop app is properly set up. 99% ready for production. process is good. it's running. is it correctly reflected in the cli and docs projects?"

The desktop app has been through 27 sessions of polish (Sessions 1–27) and is running cleanly. But `apps/docs` (Nextra 4 at shipcode.shipshit.dev/docs) was a 2-month-old snapshot: no OpenRouter, agent-label table missing 3 variants, reviewer model hardcoded to Codex with no nuance, pipeline state diagram missing phases, and zero coverage of the desktop UI (Kanban, Inbox, Costs, Skills, Terminal, Settings, ProjectSettings, PlanViewer all unmentioned). `apps/cli` was fine — clean workspace, reads all shared types, no stale refs — but missing a README.

### System flow

```mermaid
flowchart LR
  User[New visitor] -->|lands on| Web[apps/web marketing]
  Web -->|reads the docs| Docs[apps/docs Nextra 4]
  User -->|installs CLI| CLI[apps/cli]
  CLI -->|reads shared types| Shared[@shipcode/shared]
  Docs -->|references| Shared
  Docs -->|cross-links to| Desktop[apps/desktop components]
  Router[agents/github/model-router.ts] -->|5 label routes| Docs
```

### Affected surfaces

| Surface | Prior state | Now |
|---------|-------------|-----|
| apps/cli | Correct, no README | Correct + README |
| apps/docs | ~6 pages, OpenRouter-blind, desktop-blind | 19 pages, OpenRouter dedicated page, full desktop section |
| apps/web | Marketing copy (not touched) | Audit only — flagged gaps for Vincent's approval |

### Workflow

- Launched 2 Explore agents in parallel (one per app) to produce baseline audit reports
- Wrote plan file `/Users/decod3rs/.claude-genfeedai/plans/enumerated-shimmying-lightning.md` with 3 tracks (A = CLI README, B = OpenRouter + AgentType reality, C = Desktop walkthrough)
- Dispatched `codex:codex-rescue` for adversarial review of the plan; codex backgrounded without returning a final message — re-ran verifications manually via Bash/Grep/Read tools
- Adversarial findings folded into the plan before ExitPlanMode:
  - Corrected "OpenRouter is completely undocumented" → "one passing mention in index.mdx:39, no dedicated coverage"
  - Corrected "reviewer is configurable in Settings" → "configurable at onboarding only; SettingsPanel.tsx:332 only exposes executorModel"
  - Merged duplicate "Dashboard" + "Mission Control" pages after finding DashboardView.tsx:228 uses "Mission Control" as its heading
  - Added node:sqlite spot-check to verification steps
- User approved all 3 tracks, scoped Track A down to just the CLI README (install copy stays untouched — cask + npm will be published later)

### What was done

**Track A — CLI README (1 new file):**
- [x] `apps/cli/README.md` — prerequisites, commands table, env vars, local-dev invocation, executor map, architecture pointer, cross-links to /docs/cli

**Track B — OpenRouter + AgentType reality (6 files):**
- [x] `apps/docs/content/configuration.mdx` — agent label table expanded to all 5 variants (`agent:claude`, `agent:codex`, `agent:openrouter`, `agent:openrouter/auto`, `agent:openrouter/free`); added source-of-truth link to `packages/agents/src/github/model-router.ts`
- [x] `apps/docs/content/index.mdx` — intro mentions OpenRouter label variants; Review column softened to "Codex (high reasoning) — set during onboarding"
- [x] `apps/docs/content/pipeline/overview.mdx` — same softening; state diagram expanded to `Todo → Planning → Reviewing ⇄ Revising → (Awaiting Approval) → Executing → Verifying → Shipping → Completed`; cited `PipelinePhase`/`ThreadStatus` types
- [x] `apps/docs/content/openrouter.mdx` **(new)** — enabling, `OPENROUTER_API_KEY`, label variants, per-phase `AppSettings` fields, Tier 1/2/3 summary with PR #9 link, troubleshooting
- [x] `apps/docs/content/_meta.tsx` — registered `desktop` + `openrouter` nav entries
- [x] `architecture.mdx:28` node:sqlite claim verified against `packages/db/src/index.ts:1` → no edit needed

**Track C — Desktop walkthrough (11 new files + 1 edit):**
- [x] `apps/docs/content/desktop/_meta.tsx` (new section nav)
- [x] `apps/docs/content/desktop/overview.mdx` (layout tour + cross-links)
- [x] `apps/docs/content/desktop/kanban.mdx` (columns, phase sub-sections, drag-to-launch, list view, `board` quick-link)
- [x] `apps/docs/content/desktop/mission-control.mdx` (KPI strip, activity feed, click-through, notifications)
- [x] `apps/docs/content/desktop/inbox.mdx` (triage queue, filters, Go-to-issue)
- [x] `apps/docs/content/desktop/costs.mdx` (per-phase token/USD tracking, drill-through, invoice divergence caveat)
- [x] `apps/docs/content/desktop/skills.mdx` (PRD/plan/review/execute/verify skill editor, repo-scoped vs built-in)
- [x] `apps/docs/content/desktop/terminal.mdx` (auto-open, thread switcher dropdown, agent coloring, live cost estimate)
- [x] `apps/docs/content/desktop/settings.mdx` (full settings table with "onboarding-only today" flag on reviewer/planner/verifier)
- [x] `apps/docs/content/desktop/project-settings.mdx` (Session 27 githubProjectUrl override, accepted URL shapes, validator location)
- [x] `apps/docs/content/desktop/plan-viewer.mdx` (inline vs full-screen `PlanViewer`, structured sections, review pairing)
- [x] `apps/docs/content/getting-started.mdx` — "Your First Pipeline" steers into Kanban + Terminal + Mission Control; added "Next steps" cross-link block

**Bonus audit — apps/web (no edits):**
- [x] Audited Hero, HowItWorks, FeatureGrid, PlanReviewLoop, CTASection, InstallCommand, Header, Footer
- Findings: factually correct but OpenRouter-invisible and desktop-app-blind. Marketing copy is brand-critical; flagged gaps for Vincent's approval, no edits made

### Verification

| Check | Result |
|-------|--------|
| `cd apps/docs && bun run build` | ✓ 19/19 static pages, Turbopack + TypeScript clean |
| Full monorepo `bun run typecheck` | ✓ 14/14 packages |
| Grep: `agent:claude\|agent:codex\|agent:openrouter` in apps/docs/content | All 5 label variants present |
| Grep: `OPENROUTER\|OpenRouter\|openrouter` in apps/docs/content | Dedicated page + cross-links + table entries |
| Cross-check `packages/agents/src/github/model-router.ts:19-33` | Matches docs table exactly |
| Spot-check `packages/db/src/index.ts:1` | Still `import { DatabaseSync } from 'node:sqlite'` — architecture.mdx claim stays |

### Key decisions

- **Decision:** Install copy in `index.mdx` / `getting-started.mdx` / `cli/index.mdx` stays untouched
  - **Rationale:** User will publish cask + npm package later; docs intentionally describe end state. Per Vincent's scope call: "no need to delete the docs shit, nobody is reading it anyway"
- **Decision:** Dashboard and Mission Control collapsed into one `mission-control.mdx` page in Track C
  - **Rationale:** Adversarial verification against `DashboardView.tsx:228` found they're the same surface — the heading literally says "Mission Control"
- **Decision:** Settings page explicitly flags reviewer/planner/verifier as "onboarding-only today"
  - **Rationale:** Adversarial verification found `SettingsPanel.tsx:332` only exposes `executorModel`; being honest about the gap is better than implying a non-existent UI
- **Decision:** `apps/web` marketing copy left alone despite OpenRouter-blindness
  - **Rationale:** Marketing is brand-critical and the Opus/Codex pitch is deliberate. Suggesting changes without approval would overreach

### Mistakes and fixes

- **Mistake:** `codex:codex-rescue` adversarial review backgrounded itself and returned "task running in background" without a final message. The output log only captured shell commands, not the assistant's synthesis → **Fix:** Ran the same verifications manually via Bash/Grep/Read in parallel. Took ~60s and surfaced the same class of findings. Lesson: codex-rescue is good for depth but unreliable for quick "verify my plan" passes — direct verification is faster
- **Mistake:** Plan's first draft claimed OpenRouter was "completely undocumented" — overstated. Actual state was one passing mention → **Fix:** Adversarial review caught it; plan softened to "one passing mention, no dedicated coverage" before ExitPlanMode. Lesson: always grep before claiming "zero hits"
- **Mistake:** Plan's first draft claimed reviewer model is "configurable in the Settings panel" — wrong. Actual state: configurable at onboarding only → **Fix:** Adversarial verification found only `executorModel` in SettingsPanel.tsx. Plan recommendation revised from "soften to Configurable" to "soften to Codex by default (set during onboarding)"

### Root causes worth remembering

- **Marketing docs and product docs drift at different rates.** Product docs (apps/docs) should be source-of-truth-aligned and updated per-feature. Marketing copy (apps/web) should preserve brand narrative; technical updates should be explicit design calls
- **Codex rescue is not a one-shot tool for plan review.** It is optimized for investigation and depth, not quick verification. For "check my plan" use direct grep/read — it's faster and doesn't risk the background-task failure mode
- **Settings UI drift is sneaky.** The `AppSettings` type in `packages/shared/src/types.ts` can have fields that the runtime Settings panel doesn't expose. When documenting settings, always verify the UI surface, not just the type
- **"Nobody reads the docs anyway" is self-fulfilling.** The reason nobody reads them is because they're stale. Fixing them is the precondition for anyone reading them

### Files changed

**New (13):**
- `apps/cli/README.md`
- `apps/docs/content/openrouter.mdx`
- `apps/docs/content/desktop/_meta.tsx`
...
- `apps/docs/content/desktop/costs.mdx`
- `apps/docs/content/desktop/skills.mdx`
- `apps/docs/content/desktop/terminal.mdx`
- `apps/docs/content/desktop/settings.mdx`
- `apps/docs/content/desktop/project-settings.mdx`
- `apps/docs/content/desktop/plan-viewer.mdx`
...
- [ ] Commit the 18 docs/CLI files as a single feature commit (track as "docs: align with production-ready desktop app")
- [ ] Visual pass: take screenshots for each desktop/*.mdx page, add to `apps/docs/public/screenshots/`, reference from the pages
- [ ] Follow-up (separate): add reviewer/planner/verifier controls to `SettingsPanel.tsx` so the "onboarding-only today" note can be removed
- [ ] Follow-up (separate): decide whether apps/web should get an OpenRouter mention + desktop preview section; flagged in this session but needs brand approval
- [ ] Publish cask + npm package — when done, install instructions in docs immediately become truthful without edits
- [ ] Retrofit existing `main/ipc.ts` handlers to use shared `clampError` (carryover from Session 27)

### Plan file

`/Users/decod3rs/.claude-genfeedai/plans/enumerated-shimmying-lightning.md` — contains the full plan, adversarial review log, execution log, and out-of-scope items
- [ ] Commit: 14 files across 3 packages. Single commit since all changes form one feature
- [ ] Follow-up (out of scope): retrofit existing `main/ipc.ts` handlers to use shared `clampError` instead of raw `throw new Error`
- [ ] Follow-up (out of scope): consider promoting modal to `/project/:id/settings` route once surface grows past ~5 fields

---

## Session 28: Stale Inbox Notification Fix + Inline Plan Formatting
...
|-------|------------|
| Frontend | `apps/desktop/src/renderer/components/IssueDetail.tsx` |
| Backend | `apps/desktop/src/main/pipeline-bridge.ts`, `apps/desktop/src/main/notification-service.ts` |
| Data | `packages/db/src/queries/notifications.ts` |

### What was done

- [x] Diagnosed stale inbox notification: FAILED notification persists after thread transitions to `awaiting_approval` because there was no auto-dismissal on pipeline restart
- [x] Added `dismissByThread(threadId)` SQL method to `NotificationsQueries`
- [x] Exposed `dismissByThread()` on `NotificationService`
- [x] Wired auto-dismiss in `pipeline-bridge.ts`: when phase transitions to `planning`, all stale notifications for that thread are cleared before the new run fires new ones
- [x] Fixed inline plan card expansion: was showing raw JSON instead of formatted plan; now uses `resolveClientSidePlan` fallback (same logic as full-screen dialog), and falls back to `resolveRawPlanText` (not raw JSON) when parsing fails

### Files changed

- `packages/db/src/queries/notifications.ts` — added `dismissByThread(threadId)` method
- `apps/desktop/src/main/notification-service.ts` — added `dismissByThread()` wrapper that calls DB method + refreshes badge
- `apps/desktop/src/main/pipeline-bridge.ts` — on `planning` phase, call `dismissByThread` before firing new notifications
- `apps/desktop/src/renderer/components/IssueDetail.tsx` — inline expanded plan uses `resolveClientSidePlan` + `resolveRawPlanText` fallback instead of raw `plan.rawOutput`

### Key decisions

- **Decision:** Auto-dismiss on `planning` phase (not on `queued` or other early states)
  - **Context:** `planning` is the first phase that actually emits from the pipeline runner; `queued` is a UI state set before pipeline starts
  - **Rationale:** Safest signal that a new run has genuinely started; avoids premature dismissal

- **Decision:** Use IIFE pattern `{isExpanded && (() => { ... })()}` in JSX for inline plan parsing
  - **Context:** Need to compute `inlineDisplayPlan` only when expanded, without extracting a helper component
  - **Rationale:** Keeps logic co-located with render, avoids prop-drilling or state for a one-off computation
...
- [ ] Manual QA: run a pipeline that fails, verify FAILED notification appears; re-run the same thread, verify notification auto-clears when planning starts and new `awaiting_approval` notification appears
- [ ] Manual QA: expand a plan card inline → should show formatted `PlanViewer` not raw JSON
- [ ] Build `packages/db` to clear TS error on `notification-service.ts:152` (`dismissByThread` missing from dist types)

---

## Session 29: Plan full-screen dialog — readable fallback, close button, Approve & Execute, parser fix
...
```mermaid
flowchart TD
  rawOutput["rawOutput (NDJSON)"] -->|resolveRawPlanText| llmText["LLM text (fenced JSON)"]
  llmText -->|resolveClientSidePlan| clientPlan["ShipCodePlan | null"]
  plan.structured -->|already parsed| structuredPlan["ShipCodePlan"]
  clientPlan -->|fallback| PlanViewer
  structuredPlan -->|preferred| PlanViewer
  llmText -->|if both null| ProseDiv["readable prose fallback"]
  Dialog --> CloseX["X button (header)"]
  Dialog --> Footer["Approve & Execute footer\n(canApprove && isLatest)"]
```

### Root causes found

1. **NDJSON shown instead of readable text** — `rawOutput` is the full Claude CLI NDJSON stream; `resolveRawPlanText` now extracts the `result` field from the final `{"type":"result"}` line.
2. **`structured` null despite fenced block present** — `StreamParser.extractFencedBlock()` regex `[\s\S]*?` terminated early at ` ```ts ` code fences inside JSON string values (e.g. step descriptions). Fixed: require `\n` before closing fence.
3. **Old plans still showed raw JSON** — Added `resolveClientSidePlan()` in IssueDetail to re-parse `rawOutput` through Zod client-side; old plans with `structured = null` now render as `PlanViewer`.
4. **No close button** — Added X `Button` in header.
5. **No approve shortcut** — Added `DialogFooter` with "Approve & Execute" when `canApprove && fullScreenIsLatest`.
...
|-------|------------|
| Frontend | `apps/desktop/src/renderer/components/IssueDetail.tsx` |
| Primitives | `packages/ui/src/primitives/dialog.tsx` |
| Pipeline util | `packages/agents/src/stream-parser.ts` |

### What was done

- [x] Added `resolveRawPlanText(raw)` helper — extracts LLM text from NDJSON (Claude `result` field or Codex `item.completed` texts)
- [x] Added `resolveClientSidePlan(rawOutput)` helper — fenced-block regex + Zod parse; returns `ShipCodePlan | null`
- [x] Fixed `StreamParser.extractFencedBlock()` regex — `\n` before closing fence prevents early match on nested ` ```ts ` in JSON strings
- [x] Updated `planFullScreenDialog`: uses `structured ?? clientPlan`; prose fallback only if both null
- [x] Added X close button to dialog header (flex-row layout, `mb-0` override)
- [x] Added `DialogFooter` with "Approve & Execute" (closes dialog + calls `handleApprove`)
- [x] Changed `DialogOverlay` to `bg-black/70 backdrop-blur-sm` globally
- [x] Added `bg-primary` to plan full-screen `DialogContent`
- [x] 256 agents package tests pass; 14/14 typecheck green

### Files changed

- `apps/desktop/src/renderer/components/IssueDetail.tsx` — `resolveRawPlanText`, `resolveClientSidePlan`, updated `planFullScreenDialog` (X button, footer, client-side parsing, `bg-primary`); imported `DialogFooter`, `ShipCodePlan`, `shipCodePlanSchema`
- `packages/ui/src/primitives/dialog.tsx` — overlay `bg-black/70 backdrop-blur-sm`
- `packages/agents/src/stream-parser.ts` — fenced-block regex fix (`\n` before closing fence, `[^\n]*` on opening fence line)

### Key decisions

- **Decision:** Client-side re-parse in renderer rather than DB migration for old plans
  - **Context:** Old DB rows have `structured = null`; pipeline fix only helps future plans
  - **Rationale:** No migration needed; renderer tries Zod parse opportunistically

- **Decision:** Require `\n` before closing fence instead of greedy regex
  - **Context:** Non-greedy `[\s\S]*?` stopped at first ` ``` ` in JSON string values
  - **Rationale:** Markdown spec: closing fence must be on its own line; enforces this without breaking existing tests

- **Decision:** "Approve & Execute" only shows when `fullScreenIsLatest && canApprove`
  - **Rationale:** Guards against approving a stale/old plan version

### Mistakes and fixes

- **Mistake:** First pass showed NDJSON in fallback → **Fix:** `resolveRawPlanText` extracts `result` field
- **Mistake:** After extraction, plan still showed raw fenced JSON (not PlanViewer) because `structured` was null — regex terminated early at ` ```ts ` in step description → **Fix:** stream-parser regex requires `\n` before closing fence

### Next steps

- [ ] Manual QA: open full-screen dialog on old plan (`structured = null`) → should render PlanViewer, not raw JSON
- [ ] Manual QA: retry a plan with code examples in steps → new plan should parse correctly into PlanViewer
- [ ] Manual QA: X button closes dialog; Approve & Execute appears only when `awaiting_approval`
- [ ] Commit these 3 files (IssueDetail, dialog.tsx, stream-parser.ts)

---

## Session 30: Kanban card icon color consistency
...
- [x] Made the `>` (ChevronRight) open-detail icon follow the card's tone color: `text-danger/60 hover:text-danger` on failed cards, `text-warning/60 hover:text-warning` on awaiting-approval cards, `text-agent/60 hover:text-agent` on active agent cards, `text-muted hover:text-primary` on idle/todo/completed cards

### Files changed

- `packages/ui/src/KanbanBoard.tsx` — replaced static `text-muted hover:text-primary` on ChevronRight with dynamic `cn(...)` driven by `isFailed`, `isAwaiting`, `isActive`

### Key decisions

- **Decision:** Color the `>` chevron, not the retry icon
  - **Context:** Original ask was "all icons same color"; first attempt made retry icon muted — user clarified they wanted the chevron to follow card color instead
  - **Rationale:** Retry icon's danger color is intentional (signals a failed state action); chevron is passive navigation and should inherit the card's visual tone for consistency

### Mistakes and fixes

- **Mistake:** Initially made retry icon muted to match chevron -> **Fix:** Reverted, then made chevron dynamic instead
...
## Session 34: Mega session — docs + CLI rename + release workflow + web + Projects v2 sync

**Status:** Complete (all committed to master + develop; not pushed)

### System Flow

```mermaid
flowchart LR
  subgraph "Track A-C: Docs"
    CLI_README[apps/cli/README.md] --> Docs[apps/docs 19 pages]
    Docs --> OR_Page[openrouter.mdx]
    Docs --> Desktop[desktop/* 9 pages]
  end
  subgraph "CLI rename + release"
    PKG[package.json → @shipshitdev/shipcode] --> Turbo[turbo filter update]
    WF[release.yml → manual dispatch] --> OIDC[npm OIDC Trusted Publisher]
  end
  subgraph "Web"
    Hero[Hero.tsx +OR mention] --> FG[FeatureGrid +4th card]
    FG --> CW[CodeWindow OR demo line]
  end
  subgraph "Projects v2 sync"
    Idem[gh-cli.ts idempotent] --> Backfill[sync-to-project-board IPC]
    Backfill --> PSM[ProjectSettingsModal button]
    Auto[auto-attach on start-issue] --> Idem
    Scope[parseGhProjectScope] --> Onboard[onboard.ts]
    Scope --> HB[HealthBanner.tsx]
  end
```
...
| Layer | Components |
|-------|------------|
| Docs | `apps/docs/content/{_meta,index,getting-started,configuration,pipeline/overview,openrouter,cli/index,desktop/*}.mdx` (5 edited, 12 new) |
| CLI | `apps/cli/package.json` (rename), `apps/cli/README.md` (new), `apps/cli/src/commands/onboard.ts` (scope check) |
| Web | `Hero.tsx`, `FeatureGrid.tsx`, `CodeWindow.tsx`, `InstallCommand.tsx` |
| Release | `.github/workflows/release.yml` (manual dispatch, OIDC, removed macOS/homebrew jobs) |
| Agents | `gh-cli.ts` (idempotency), `health-check.ts` (parseGhProjectScope, hasProjectScope), `index.ts` (exports) |
| Shared | `types.ts` (GhAuthStatus.hasProjectScope), `ipc-channels.ts` (sync-to-project-board) |
| Desktop main | `ipc.ts` (sync handler + auto-attach on start-issue) |
| Desktop renderer | `ProjectSettingsModal.tsx` (sync button), `HealthBanner.tsx` (scope warning) |
| UI | `KanbanBoard.tsx` ('use client' directive) |
| Tests | `gh-cli.test.ts` (+4), `health-check.test.ts` (+7) |

### What was done

**Track A — CLI README:**
- [x] `apps/cli/README.md` — command list, env vars, local-dev, executor map, CLI vs Desktop table

**Track B — OpenRouter + AgentType reality:**
- [x] Agent label table expanded to 5 variants in `configuration.mdx`
- [x] Review model softened to "Codex by default (set during onboarding)" in `index.mdx` + `pipeline/overview.mdx`
- [x] State diagram expanded with `awaiting_approval` + `shipping` phases
- [x] New `openrouter.mdx` — enabling, env var, labels, per-phase fields, tiers, troubleshooting
- [x] `_meta.tsx` updated with `desktop` + `openrouter` nav entries
- [x] Verified `architecture.mdx:28` node:sqlite claim still correct

**Track C — Desktop walkthrough (9 pages):**
- [x] `desktop/{_meta,overview,kanban,mission-control,inbox,costs,skills,terminal,settings,project-settings,plan-viewer}.mdx`
- [x] `getting-started.mdx` rewritten desktop-first with "Next steps" cross-links

**CLI rename:**
- [x] `apps/cli/package.json` name → `@shipshitdev/shipcode`, bin map kept as `shipcode`
- [x] All npx/npm refs updated across docs, web, workflow
- [x] `.github/workflows/release.yml` turbo filter → `--filter=@shipshitdev/shipcode`

**Release workflow:**
- [x] Converted from `on: release: published` to `on: workflow_dispatch` (4 inputs)
- [x] Removed `build-desktop-macos` + `update-homebrew-cask` jobs (no Apple signing secrets), preserved as commented block
- [x] Fixed `secrets.VERCEL_ORG_ID` → `vars.VERCEL_ORG_ID` bug
- [x] Switched publish-cli from NPM_TOKEN to OIDC Trusted Publisher (`--provenance`, `id-token: write`)

**apps/web OpenRouter value prop:**
- [x] Hero subhead: added "with Claude, Codex, or any OpenRouter model"
- [x] FeatureGrid: 4th card "Any Model, Any Phase" with Shuffle icon, layout → `lg:grid-cols-4`
- [x] CodeWindow: demo line → `Label: agent:openrouter/auto`

**KanbanBoard fix:**
- [x] Added `'use client'` to `packages/ui/src/KanbanBoard.tsx` (was breaking `apps/web` Next.js 16 SSR build)

**GH issue #26:**
- [x] Created shipshitdev/shipcode#26 — expose reviewer/planner/verifier model selectors in SettingsPanel

**Projects v2 sync (4 patches):**
- [x] Patch 1: `addIssueToProject` made idempotent — catches "already in project" stderr, returns `{added, alreadyPresent}`. 4 new tests
- [x] Patch 2: New `github:sync-to-project-board` IPC handler + "Sync existing issues to board" button in ProjectSettingsModal. Walks all cached issues, accumulates `{attached, alreadyPresent, failed, errors}`. Per-issue error breakdown (first 5)
- [x] Patch 3: Auto-attach on `github:start-issue` (best-effort after pipeline spawn, mirrors `github:create-issue` pattern)
- [x] Patch 4: `parseGhProjectScope()` + `GhAuthStatus.hasProjectScope`. Wired into CLI onboard (prints fix command) and HealthBanner (surfaces alongside other auth warnings). 7 new tests

**Worktree cleanup:**
- [x] Deleted 2 failed pipeline worktrees (`K6cgYKC7ManolQfQeeHqC` + `UpA3XrqxhKV60N_kPJkOv`) + their DB thread rows (CASCADE cleaned plans/diffs/etc)
- [x] Deleted corresponding `shipcode/*` branches
...
- `apps/cli/README.md`
- `apps/docs/content/openrouter.mdx`
- `apps/docs/content/desktop/{_meta.tsx,overview,kanban,mission-control,inbox,costs,skills,terminal,settings,project-settings,plan-viewer}.mdx`

**Modified (20+):**
- `apps/cli/package.json`, `apps/cli/src/commands/onboard.ts`
- `apps/docs/content/{_meta.tsx,index,getting-started,configuration,pipeline/overview,cli/index}.mdx`
- `apps/web/app/components/{Hero,FeatureGrid,CodeWindow,InstallCommand}.tsx`
- `.github/workflows/release.yml`
- `packages/agents/src/github/{gh-cli.ts,gh-cli.test.ts}`, `packages/agents/src/{health-check.ts,health-check.test.ts,index.ts}`
- `packages/shared/src/{types.ts,ipc-channels.ts}`
- `apps/desktop/src/main/ipc.ts`
- `apps/desktop/src/renderer/components/{ProjectSettingsModal,HealthBanner}.tsx`
- `packages/ui/src/KanbanBoard.tsx`

### Key decisions

- **Decision:** CLI published as `@shipshitdev/shipcode`, not unscoped `shipcode`
  - **Context:** shipshitdev org already owns `@shipshitdev/ui` on npm
  - **Rationale:** Namespace protection + consistency. `bin` map still exposes `shipcode` on PATH after global install
- **Decision:** Release workflow converted to manual-dispatch only
  - **Context:** macOS codesigning secrets not configured; automated release would always skip the macOS build + Homebrew cask update
  - **Rationale:** Manual dispatch with boolean toggles per job lets Vincent test individual jobs without the all-or-nothing release trigger
- **Decision:** Switched from NPM_TOKEN to OIDC Trusted Publisher
  - **Context:** Vincent already has OIDC configured for `@shipshitdev/ui` via `publish.yml`
  - **Rationale:** No secret to rotate, SLSA provenance statement included, aligns with existing org practice
- **Decision:** Projects v2 sync button placed inside ProjectSettingsModal, not Kanban header
  - **Context:** User confirmed during plan review
  - **Rationale:** Pairs with the githubProjectUrl input it depends on; keeps Kanban header lean
- **Decision:** Install copy in docs left untouched (brew tap + npx blocks)
  - **Context:** Cask + npm package not yet published
  - **Rationale:** Vincent: "no need to delete the docs shit. nobody is reading it anyway. i will publish both later"
- **Decision:** apps/web marketing copy updated minimally for OpenRouter
  - **Context:** Hero/FeatureGrid/HowItWorks/PlanReviewLoop were Opus+Codex only
  - **Rationale:** Added OpenRouter as a "flexibility" layer (Hero subhead + 4th FeatureGrid card + CodeWindow demo line) without disrupting the core adversarial-review brand narrative

### Mistakes and fixes

- **Mistake:** Plan's first draft claimed OpenRouter was "completely undocumented" in apps/docs → **Fix:** Adversarial review (manual, after codex:rescue backgrounded without a final message) found one passing mention at `index.mdx:39`. Softened to "one passing mention, no dedicated coverage"
- **Mistake:** Plan claimed reviewer model is "configurable in the Settings panel" → **Fix:** Adversarial verification found `SettingsPanel.tsx:332` only exposes `executorModel`. Plan revised to "Codex by default (set during onboarding)"
- **Mistake:** Plan claimed brew tap name in docs was wrong (`shipshitdev/shipcode` vs `shipshitdev/tap`) → **Fix:** Later discovered BOTH tap repos exist. `github.com/shipshitdev/homebrew-shipcode` has `Casks/shipcode.rb` template matching the docs; `github.com/shipshitdev/homebrew-tap` is the old one with `meterbar.rb`. Docs copy was actually correct. Logged correction in plan file
- **Mistake:** Commit `081fc65` (KanbanBoard 'use client') accidentally swept in Vincent's in-flight Session 30 chevron-tone work via `git add` — **Fix:** Flagged to Vincent; he said leave it. Not split
- **Mistake:** OIDC commit `f71d39d` landed on `develop` instead of `master` because Vincent had switched branches mid-session → **Fix:** Vincent merged it into master via PR #28

### Root causes worth remembering

- **Codex rescue is unreliable for quick plan-review passes.** Third incident now (sessions 27, 33, 34). It backgrounds, wedges after 5+ minutes of broad scanning, and sometimes never returns a final message. Direct grep/read verification is faster and more reliable for "check my plan" use cases
- **`git add <file>` stages ALL local changes in that file, including someone else's uncommitted work.** Should have used `git add -p` or `git diff --cached --stat` before committing to catch the sweep
- **Settings UI can drift from AppSettings type silently.** The type has fields that no runtime UI exposes (reviewer/planner/verifier). When documenting settings, always verify the UI surface, not just the type definition

### Next steps

- [ ] Push master to origin — 7+ commits ahead, all verified green
- [ ] Manual QA: open ProjectSettingsModal → paste `https://github.com/orgs/shipshitdev/projects/1` → click "Sync existing issues to board" → verify issues land on the GitHub board
- [ ] Manual QA: drag a Todo card into Planning → check board → verify auto-attach works
- [ ] Manual QA: run `shipcode onboard` → verify `project` scope check prints ✓ or ⚠
- [ ] Set up npm Trusted Publisher on npmjs.com: configure `@shipshitdev/shipcode` → Repository: `shipshitdev/shipcode`, Workflow: `release.yml`
- [ ] Test manual-dispatch release: `gh workflow run release.yml -f release_tag=v0.1.0 -f deploy_web=true -f build_linux=false -f publish_cli=true`
- [ ] Delete `NPM_TOKEN` from GH Actions repo secrets (no longer consumed)
- [ ] Fix Vincent's IssueDetail.tsx `pendingAction` refactor — 3 failing tests in IssueDetail.test.tsx (not this session's work but blocking green desktop tests)
- [ ] Screenshots for the 9 desktop docs pages (needs running app + visual pass)
- [ ] Wire reviewer/planner/verifier selects into SettingsPanel (shipshitdev/shipcode#26)
